...
// ── BACKLOG_MODE guard ────────────────────────────────────────────────────────
// When BACKLOG_MODE=1 is set in Railway → Variables the server skips all
// new-token discovery and wallet-collection-job processing so every Helius
// credit goes toward clearing the existing hollow-wallet backlog.
//
// What is disabled:
//   • ProcessJobsScheduler  — stops picking up wallet_collection_jobs for new tokens
//   • PostLaunchWatcher     — stops the authority-transition WebSocket monitor
//   • TokenDiscovery        — stops the Pump.fun WebSocket subscription
//
// What keeps running:
//   • PriceRefreshScheduler        — keeps position values current
//   • RescoreScheduler             — keeps win_rate / intelligence_score fresh
//   • EnrichUnenrichedScheduler    — the sole consumer of Helius credits
//   • HeliusCuLogRetentionScheduler
//
// To resume normal operation: remove BACKLOG_MODE from Railway Variables and
// redeploy (or just delete the variable — Railway picks it up on next deploy).
// ─────────────────────────────────────────────────────────────────────────────
const BACKLOG_MODE = process.env.BACKLOG_MODE === "1";

if (BACKLOG_MODE) {
  console.log(
    "[server] ⚡ BACKLOG_MODE=1 — TokenDiscovery, PostLaunchWatcher, and " +
    "ProcessJobsScheduler are DISABLED. All Helius capacity goes to hollow-wallet enrichment.",
  );
} else {
