...
// P1-B: Helius key liveness check — makes one cheap getSlot call to confirm
// the key is valid before any scheduler tries to use it. Non-blocking: emits
// a CRITICAL log entry but does not crash the process so HTTP serving and the
// non-Helius schedulers (price-refresh, leaderboard) stay up during an outage.
void (async () => {
  const heliusKey = process.env.HELIUS_API_KEY;
  if (!heliusKey) return; // already flagged by the env-var audit above
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `[server] ‼ Helius key validation FAILED — HTTP ${res.status}: ${body}. ` +
        "PostLaunchWatcher and TokenDiscovery will not function. " +
        "Check HELIUS_API_KEY in Railway Variables.",
      );
      return;
    }
    ...
    if (json.error) {
      console.error(
        `[server] ‼ Helius key validation FAILED: ${json.error.message}. ` +
        "PostLaunchWatcher and TokenDiscovery will not function.",
      );
    } else {
      console.log(`[server] ✓ Helius key validated — current slot ${json.result}`);
    }
  } catch (err) {
    console.error(
      `[server] ‼ [CRITICAL] Helius connectivity check failed (timeout or network error): ` +
      `${err instanceof Error ? err.message : String(err)}. ` +
      "PostLaunchWatcher and TokenDiscovery may not function — check HELIUS_API_KEY and network.",
    );
  }
})();
...
function handleApiRoute(request: Request, pathname: string): Promise<Response> | Response | null {
  ...
  // Runtime health check — CRON_SECRET required (returns internal system state:
  // env var flags, Helius budget, scheduler liveness, DB health).
