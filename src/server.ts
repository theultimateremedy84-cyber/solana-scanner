...
// PATCH FIX (scoring-patch v1): Trigger one-time rescore on boot so all 835
// wallets are classified with the corrected 0-100 score scale and win_rate
// logic without requiring a manual POST to /api/rescore-wallets.
// Runs async after a 10 s warmup delay — never blocks the HTTP server.
startRescoreOnBoot();
