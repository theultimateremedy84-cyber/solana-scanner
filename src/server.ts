...
import { startTokenDiscovery, TokenDiscovery } from "./lib/api/token-discovery";
...
// P2 AUDIT FIX: alerts resolution endpoint — marks open alerts as resolved
// so the 559+ critical alerts don't accumulate forever.
// Requires migration 20260720000002_alerts_resolution.sql.
import { handleResolveAlertGet, handleResolveAlertPost } from "./lib/api/resolve-alert-handler";
// AUDIT FIX (globalThis budget reset): persist Helius daily CU counter to DB
// so Railway restarts do not zero it and allow re-burning the full quota.
// Requires migration 20260718000002_helius_budget_daily.sql.
import { initHeliusBudgetPersistence } from "./lib/api/helius-budget-persistence";

// AUTONOMOUS AGENT (2026-07-19): self-healing pipeline monitor v2
// Polls /api/monitor-dashboard, detects issues, applies fixes with
// circuit-breaker + cooldown logic, and sends SMTP email alerts.
// Requires migrations 20260716000020 and 20260719000001 to be applied first.
import {
  handleAgentStatus,
  handleAgentIssues,
  handleAgentRun,
  handleAgentReports,
  handleAgentReportById,
  handleAgentSettingsGet,
  handleAgentSettingsPut,
  handleAgentCircuits,
  handleCircuitReset,
  handleAgentFixLog,
  startAgentScheduler,
} from "./lib/api/agent-handler";
...
// AUDIT FIX: restore persisted Helius daily CU counter from DB before any
// scheduler starts consuming credits — must be awaited so the budget object
// is seeded before _consumeHC() runs for the first time.
initHeliusBudgetPersistence().catch((err) =>
  console.warn("[server] Helius budget persistence init failed (non-fatal):", err),
);
...
// AUTONOMOUS AGENT (2026-07-19): start the self-healing agent polling loop.
// Loads interval_minutes from agent_settings (default 5 min). Runs silently
// if monitoring_enabled=false in settings — no crash, no noise.
startAgentScheduler().catch((err: unknown) => {
  console.error("[server] ‼ FATAL-AVOIDED \"AgentScheduler\" failed to start:", err);
});
...
// ---------------------------------------------------------------------------
// SECURITY FIX: shared helper — validates the x-cron-secret header.
// Returns a 401 Response if auth fails, or null if the request is allowed.
// CRON_SECRET is required; if the env var is missing every request is rejected.
// ---------------------------------------------------------------------------
...
function handleApiRoute(request: Request, pathname: string): Promise<Response> | Response | null {
  ...
  // SECURITY FIX: /api/discovery-status now requires CRON_SECRET.
  // Previously exposed pipeline internals (env vars, scheduler state,
  // Helius budget, enrichment backlog) to any unauthenticated caller.
  ...
  // as /api/funding-clusters).
  if (pathname === "/api/backlog-status" && request.method === "GET") {
    return handleBacklogStatusGet();
  }
  // SECURITY FIX: /api/monitor-dashboard now requires CRON_SECRET.
  // Previously exposed 22 sections of live pipeline metrics (wallet counts,
  // Helius CU budget, enrichment coverage, scheduler state) without any auth.
  if (pathname === "/api/monitor-dashboard" && request.method === "GET") {
    const authError = requireCronSecret(request);
    if (authError) return authError;
    return handleMonitorDashboardGet();
  }
  // P2 AUDIT FIX: alerts resolution — GET returns open-alert counts (no auth),
  // POST marks one or many alerts resolved (requires x-cron-secret).
  if (pathname === "/api/resolve-alert") {
    if (request.method === "GET")  return handleResolveAlertGet();
    if (request.method === "POST") return handleResolveAlertPost(request);
    return new Response(
      JSON.stringify({ error: "Method not allowed" }),
      { status: 405, headers: { "Content-Type": "application/json" } },
    );
  }
  // AUTONOMOUS AGENT (2026-07-19): all agent routes require CRON_SECRET.
  // Routes: status, issues, run, reports, settings, circuits, fix-log.
  // Also passes /api/discovery-control through (used by agent-fixer to
  // pause/restart token discovery when Helius budget is exhausted).
  if (pathname.startsWith("/api/agent/") || pathname === "/api/discovery-control") {
    const authError = requireCronSecret(request);
    if (authError) return authError;

    if (pathname === "/api/agent/status" && request.method === "GET")
      return handleAgentStatus(request);
    if (pathname === "/api/agent/issues" && request.method === "GET")
      return handleAgentIssues(request);
    if (pathname === "/api/agent/run" && request.method === "POST")
      return handleAgentRun(request);
    if (pathname === "/api/agent/reports" && request.method === "GET")
      return handleAgentReports(request);
    if (pathname.startsWith("/api/agent/reports/") && request.method === "GET") {
      const reportId = pathname.slice("/api/agent/reports/".length);
      if (reportId && !reportId.includes("/")) return handleAgentReportById(request, reportId);
    }
    if (pathname === "/api/agent/settings") {
      if (request.method === "GET") return handleAgentSettingsGet(request);
      if (request.method === "PUT") return handleAgentSettingsPut(request);
      return new Response(
        JSON.stringify({ error: "Method not allowed" }),
        { status: 405, headers: { "Content-Type": "application/json" } },
      );
    }
    if (pathname === "/api/agent/circuits" && request.method === "GET")
      return handleAgentCircuits(request);
    if (pathname.startsWith("/api/agent/circuits/") && pathname.endsWith("/reset") && request.method === "POST") {
      const category = pathname.slice("/api/agent/circuits/".length).replace(/\/reset$/, "");
      if (category) return handleCircuitReset(request, category);
    }
    if (pathname === "/api/agent/fix-log" && request.method === "GET")
      return handleAgentFixLog(request);

    // FIX (2026-07-20): /api/discovery-control was matched by this block but fell
    // through to the 404 "Unknown agent route" response — the handler was never
    // implemented. agent-fixer calls this to pause/restart TokenDiscovery when
    // the Helius budget is critical. Now handled inline here.
    // NOTE: handleApiRoute is not async (callers check `!== null` without await),
    // so we return a Promise<Response> directly using .then()/.catch() chaining
    // rather than using the await keyword.
    if (pathname === "/api/discovery-control" && request.method === "POST") {
      return request.json().then((body: unknown) => {
        const action = (body as { action?: string })?.action;
        if (action === "pause") {
          TokenDiscovery.getInstance().stop();
          console.log("[server] /api/discovery-control pause — TokenDiscovery stopped by agent");
        ...
        if (action === "restart") {
          // FIX (2026-07-20): start() has an `if (this.running) return` guard.
          // When the WebSocket is dead but the instance is still marked running=true
          // (the normal state when a WS connection drops mid-session), calling
          // start() is a silent no-op — the WebSocket never reconnects.
          // Fix: call stop() first to reset running=false + close any zombie WS,
          // then start() sets running=true and calls connect() fresh.
          const td = TokenDiscovery.getInstance();
          td.stop();
          td.start().catch((err: unknown) => {
            console.error("[server] /api/discovery-control restart error:", err);
          });
          console.log("[server] /api/discovery-control restart — TokenDiscovery stop()+start() called by agent");
          return new Response(JSON.stringify({ ok: true, action: "restart" }), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        }
    ...
    return new Response(
      JSON.stringify({ ok: false, error: "Unknown agent route" }),
