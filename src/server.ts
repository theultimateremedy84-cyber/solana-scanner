...
//
// TEMP PAUSE: Discovery is disabled by default so all Helius/API capacity can
// go toward clearing the wallet enrichment backlog. Set env var
// ENABLE_TOKEN_DISCOVERY=true to re-enable discovery of new tokens.
if (process.env.ENABLE_TOKEN_DISCOVERY === "true") {
  ...
  console.log(
    "[server] TokenDiscovery is PAUSED (set ENABLE_TOKEN_DISCOVERY=true to re-enable).",
  );
}
