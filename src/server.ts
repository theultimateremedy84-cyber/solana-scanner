...
// Replaces the fragile Railway cron → HTTPS self-loopback chain. Processes up
// to 5 pending jobs per tick; the Railway cron (cron-trigger.mjs) is kept as
// belt-and-suspenders but is no longer the primary execution path.
...
function handleApiRoute(request: Request, pathname: string): Promise<Response> | Response | null {
  ...
  // FIX: process-jobs was using createAPIFileRoute which is not registered by
  // @lovable.dev/vite-tanstack-config on the node-server Nitro preset.
  // Railway cron was hitting this endpoint and receiving no handler.
  ...
  // Runtime health check for the P2-A discovery system — no auth required
