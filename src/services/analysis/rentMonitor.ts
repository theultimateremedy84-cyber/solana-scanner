...
/**
 * Programs whose owned accounts are maintained by the Solana runtime itself
 * and are never subject to rent eviction. Any account whose `owner` field
 * matches one of these IDs is skipped by the rent-exempt check.
 *
 *   11111111111111111111111111111111   — System Program (wallets, fee-payers)
 *   BPFLoaderUpgradeab1111111111111111111111111 — BPF Upgradeable Loader
 ...
 *     such as ComputeBudget, Vote, Stake — all owned by NativeLoader)
 */
const SYSTEM_OWNERS = new Set<string>([
  "11111111111111111111111111111111",
  "BPFLoaderUpgradeab1111111111111111111111111",
...
export function analyzeRentExemption(
  ...
  for (const account of accounts) {
    // Guard: skip runtime-owned accounts (System Program, BPF Loaders,
    // NativeLoader — which covers ComputeBudget, Vote, Stake, etc.).
    // These accounts are maintained by the Solana runtime and are never
    // subject to rent eviction, so flagging them would be a false positive.
    if (SYSTEM_OWNERS.has(account.owner)) {
      console.log("Skipping account:", account.address, "Owner:", account.owner);
      continue;
    }

    console.log("Flagging account:", account.address, "Owner:", account.owner);
    checkedCount++;
