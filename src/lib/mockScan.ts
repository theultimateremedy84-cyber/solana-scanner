...
export type { AuthorityMonitorResult, AuthorityClassification, AuthorityDetail } from "@/services/analysis/authorityMonitor";
...
export interface ScanResult {
  ...
  // ---- Authority Monitor ----
  /**
   * Classified authority status for upgrade, mint, and freeze authorities.
   * overallClassification drives the upgradeable floor in globalRiskScore.
   */
  authorityMonitor: AuthorityMonitorResult;
...
const NAMES = [
  "MemeMaster", "LunarDoge", "ShibaSol", "PepeChain", "WifCoin",
  "BonkPup", "CatWifHat", "GigaFrog", "NootNoot", "TrumpCoin",
  "SolFloki", "MoonBull", "RocketPepe", "BasedSol", "GigaChad",
];
const SYMBOLS = [
  "MEME", "LDOGE", "SHIBSOL", "PEPEC", "WIF",
  "BONKP", "CWHAT", "GFROG", "NOOT", "TRUMP",
  "SFLOKI", "MBULL", "RPEPE", "BASED", "CHAD",
];
...
function range(r: () => number, lo: number, hi: number) {
  return lo + r() * (hi - lo);
}
...
export function scanToken(address: string): ScanResult {
  ...
  // --- Mock authority monitor (derived from existing mint/freeze flags) ---
  // In mock mode, we cannot run getAccountInfo, so we classify authorities
  // purely from the active/revoked flags. All active authorities are treated
  // as wallet-held (Unsafe/Upgradeable) to reflect the worst-case scenario.
  const mockUpgradeAuth: AuthorityMonitorResult["upgradeAuthority"] = {
    address: null,
    classification: "Immutable/Safe",
    isMultisig: false,
    reason: "Metadata update authority not checked in mock mode — defaulting to Immutable/Safe.",
  };
  const mockMintAuth: AuthorityMonitorResult["mintAuthority"] = mintActive
    ? {
        address: "MockWallet1111111111111111111111111111111111",
        classification: "Unsafe/Upgradeable",
        isMultisig: false,
        reason: "Mint authority is active and held by a wallet (mock data).",
      }
    : {
        address: null,
        classification: "Immutable/Safe",
        isMultisig: false,
        reason: "Mint authority is revoked — supply is fixed.",
      };
  const mockFreezeAuth: AuthorityMonitorResult["freezeAuthority"] = freezeActive
    ? {
        address: "MockWallet1111111111111111111111111111111111",
        classification: "Unsafe/Upgradeable",
        isMultisig: false,
        reason: "Freeze authority is active and held by a wallet (mock data).",
      }
    : {
        address: null,
        classification: "Immutable/Safe",
        isMultisig: false,
        reason: "Freeze authority is revoked — accounts cannot be frozen.",
      };

  const mockOverallClassification: AuthorityMonitorResult["overallClassification"] =
    mintActive || freezeActive ? "Unsafe/Upgradeable" : "Immutable/Safe";

  // Apply upgradeable floor to mock globalRiskScore
  const mockUpgradeableFloor = mockOverallClassification === "Unsafe/Upgradeable" && globalRiskScore < 20;
  if (mockOverallClassification === "Unsafe/Upgradeable") {
    globalRiskScore = Math.max(globalRiskScore, 20);
  }

  const authorityMonitor: AuthorityMonitorResult = {
    available: false, // mock mode — no real RPC check
    upgradeAuthority: mockUpgradeAuth,
    mintAuthority: mockMintAuth,
    freezeAuthority: mockFreezeAuth,
    overallClassification: mockOverallClassification,
    upgradeableRiskFloorApplied: mockUpgradeableFloor,
  };
  ...
  const verdictSummary =
    honeyPot === "CONFIRMED"
      ...
      : mockOverallClassification === "Unsafe/Upgradeable"
        ? `Contract is upgradeable — ${mintActive ? "mint" : "freeze"} authority is held by a single wallet and can be changed without warning.`
        : mintActive
  ...
  const riskBreakdown: RiskPhaseBreakdown = {
    onChainCode: {
      ...
      driver: honeyPot === "CONFIRMED" ? "Confirmed honeypot"
        : mockOverallClassification === "Unsafe/Upgradeable" ? "Single-wallet authority"
        : mintActive ? "Mint authority active"
        : freezeActive ? "Freeze authority active"
        : "Clean",
    },
    marketBehavior: {
  ...
  if (mockOverallClassification === "Unsafe/Upgradeable") {
    redFlags.push({
      ...
      detail: `${mintActive ? "Mint authority" : "Freeze authority"} is held by a wallet — can be modified without multisig protection.`,
    });
    if (mockUpgradeableFloor) {
      redFlags.push({
        id: "authority-floor",
        severity: "warn",
        title: "Risk floor applied: upgradeable contract",
        detail: "Global Risk Score raised to MEDIUM minimum (≥ 20) due to single-wallet authority.",
      });
    }
  }
