...
export function scanToken(address: string): ScanResult {
  ...
  // Mock never has a detected authority transition.
  // Live scans that have is_authority_transitioned = true apply a Critical Risk
  // floor of 90 in scan-core.ts instead.
  const is_authority_transitioned = false;
  // Mock never has a detected account-data-modification either. Live scans
  // with is_account_resized = true render an "Account Storage Tampered"
  // warning in the Risk Breakdown panel (see riskBreakdown.accountResize).
  ...
  const riskBreakdown: RiskPhaseBreakdown = {
    ...
    // authorityTransition is absent in mock data — only populated by live scans
    // that have been flagged by PostLaunchWatcher.
  };
