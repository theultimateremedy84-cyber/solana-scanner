...
export type {
  ...
  PriorTokenRecord,
} from "@/services/analysis/developerHistory";

export type { SimulatedInstruction } from "@/services/analysis/transactionSimulator";
export type { RentExemptViolation } from "@/services/analysis/rentMonitor";
...
export interface RiskPhaseBreakdown {
  ...
  /**
   * Phase 10: Developer History phase.
   * Present on all live scans where a creator wallet was identified and the
   * Supabase database was reachable. Drives the "Developer History" panel.
   *
   * score 0   = "Clean" — no prior rug patterns found.
   * score 35  = "Suspicious" — 1+ prior high-risk tokens (+15 penalty applied).
   * score 60  = "Serial Offender" — 2+ high-risk / 1+ EXTREME tokens (floor ≥ 60).
   * score 80  = "Confirmed Scammer" — serial EXTREME or honeypot history (floor ≥ 80).
   */
  developerHistory?: {
    score: number;
    label: string;
    /** Developer wallet address (base58). */
    developerWallet: string | null;
    /** Number of prior token launches found in the DB. */
    priorLaunchCount: number;
    /** Number of prior tokens that scored EXTREME risk. */
    extremeRiskCount: number;
    /** Developer classification tier. */
    classification: import("@/services/analysis/developerHistory").DeveloperClassification;
   ...
   * Present when simulateTransaction ran against the token's recent transactions.
   *
   * score 0   = no exploit detected (clean simulation).
   * score 100 = Swap + Authorization instructions co-bundled (CRITICAL).
   *
   * When is_atomic_exploit is true, scan-core FORCES globalRiskScore = 100.
   * The instructions array lists every instruction found in the simulation,
   * with isAuthorizationRelated ones highlighted red in the UI panel.
   */
  atomicExploit?: {
    score: number;
    label: string;
    /** True when a swap + authority-modifying instruction bundle was detected. */
    is_atomic_exploit: boolean;
    /** Full instruction list from the simulation (swap + inner CPIs). */
    instructions: import("@/services/analysis/transactionSimulator").SimulatedInstruction[];
    /** Simulation error if the RPC returned one (null = clean). */
    simulationError: string | null;
    /** Transaction signature where the exploit was found (if any). */
    signature?: string;
    /** ISO timestamp of detection. */
    detectedAt?: string;
    driver: string;
   ...
   * Present when the rent-exempt check ran against the token's recent accounts.
   *
   * score 0   = all accounts rent-exempt (safe).
   * score 80  = one or more accounts below rent-exempt minimum (CRITICAL risk).
   *
   * When has_non_rent_exempt_accounts is true, scan-core applies a scoring
   * penalty and renders an 'Account Vulnerability' warning in the UI.
   */
  rentExempt?: {
    score: number;
    label: string;
    ...
    violations: import("@/services/analysis/rentMonitor").RentExemptViolation[];
   ...
   * Present on all live scans where program bytecode was available.
   *
   * Three-phase architecture:
   *   Phase A: Static analysis (BPF opcode scan + Rust pattern matching).
   *            Flags unchecked arithmetic as "Preliminary Risk" — NOT immediately Critical.
   *   Phase B: Behavioral verification via edge-case simulation (u64::MAX+1, u64::MIN-1).
   *            If simulation panics/reverts → "Code Style Warning" (non-malicious).
   *            If simulation succeeds silently → "CONFIRMED EXPLOIT VECTOR".
   *   Phase C: Contextual reputation cross-reference.
   ...
   *
   * score 0   = no preliminary risk detected (safe).
   * score 50  = preliminary risk (static analysis only, simulation inconclusive).
   * score 75  = code style warning (simulation confirmed safe runtime guards).
   * score 100 = confirmed exploit vector (+40 pts penalty applied).
   */
  overflowMonitor?: {
    score: number;
    label: string;
    /** True when static analysis found unchecked arithmetic (Preliminary Risk). */
    is_preliminary_risk: boolean;
    /** True only when behavioral simulation confirmed the exploit (CONFIRMED EXPLOIT VECTOR). */
    is_confirmed_exploit: boolean;
    /** True when is_confirmed_exploit=true (kept for backward compat). */
    is_vulnerable: boolean;
    /**
     * Final alert classification after all 3 phases:
     * "DANGER: Intentional Backdoor Detected" | "CONFIRMED EXPLOIT VECTOR" |
     * "Technical Debt: Audit Required" | "Preliminary Risk" |
     * "Code Style Warning" | "Status: Under Review" | "Safe"
     */
    alert_tier: string;
    /**
     * How the verdict was reached — always shown in the UI.
     * "Verified via Simulation" | "Heuristic Warning" | "Status: Under Review"
     */
    verification_method: string;
    /** Number of arithmetic instructions with no surrounding bounds-check. */
    unsafe_instruction_count: number;
    /** Number of arithmetic instructions with a visible bounds-check. */
    safe_instruction_count: number;
    /** Analysis confidence — never "low" (shown as "Status: Under Review" if uncertain). */
    confidence: "medium" | "high";
    /** Simulation test results (edge-case values). */
    simulation: import("@/services/analysis/overflowMonitor").SimulationTestResult;
    /** Human-readable summary. */
    driver: string;
  };
}
...
export interface ScanResult {
  ...
  /**
   * 'Transaction Bloat & Re-routing' Monitor — max Cross-Program-Invocation
   * (CPI) nesting depth observed in any tracked transaction.
   *
   *   0   = unknown / not analysed yet
   *   1   = no CPIs (top-level only)
   *   2   = single-layer CPI (normal)
   *   ≥3  = obfuscated path → is_path_obfuscated = TRUE, +25 risk penalty
   *   ≥4  = 'Extreme Obfuscation' → Critical Risk alert (score ≥ 90)
   *
   * DB column: scan_history.cpi_depth INTEGER NOT NULL DEFAULT 0
   */
  cpiDepth: number;

  /**
   * True when the latest tracked transaction's CPI nesting depth was >= 3.
   * Deep CPI nesting is a hallmark of 'Programmable Rugs' that hide
   * malicious logic in nested program calls and cause standard scanners
   * to time out / return Unknown.
   *
   * DB column: scan_history.is_path_obfuscated BOOLEAN NOT NULL DEFAULT FALSE
   */
  is_path_obfuscated: boolean;

  /**
   * 'CPI Manipulation' Detector — TRUE when any analysed transaction
   * invokes an UNTRUSTED program via Cross-Program-Invocation (i.e.
   * the programId is not in TRUSTED_PROGRAM_LIST and not a known
   * system program). Forces globalRiskScore = 100 (CRITICAL).
   *
   * DB column: scan_history.is_cpi_manipulated BOOLEAN NOT NULL DEFAULT FALSE
   */
  is_cpi_manipulated: boolean;

  /**
   * Human-readable summary of the CPI risk, plus the suspicious program IDs.
   * Powers the "CPI Analysis" panel in the UI risk breakdown.
   *
   * DB column: scan_history.cpi_risk_details TEXT
   */
  cpi_risk_details: string | null;

  /**
   * Distinct trusted programIds invoked (for the CPI Analysis panel).
   */
  cpiTrustedProgramIds: string[];

  /**
   * Distinct UNTRUSTED programIds invoked — rendered in RED in the UI.
   */
  cpiSuspiciousProgramIds: string[];

  /**
   * Phase 14 — 'State Hijacking' Detector. TRUE when any analysed
   * transaction interacted with a PDA whose address does NOT match
   * the canonical derivation from KNOWN_SEED_MAPPINGS. Forces
   * globalRiskScore = 100 (CRITICAL).
   *
   * DB column: scan_history.is_state_hijacked BOOLEAN NOT NULL DEFAULT FALSE
   */
  is_state_hijacked: boolean;

  /**
   * Pipe-delimited human-readable summary of every hijacked PDA finding
   * (program, slot, expected address, provided address). Powers the
   * "PDA Integrity" panel in the UI risk breakdown.
   *
   * DB column: scan_history.state_hijack_details TEXT
   */
  state_hijack_details: string | null;

  /**
   * Structured list of hijacked PDA findings, used by the UI panel to
   * render the expected-vs-provided diff with copy-friendly addresses.
   */
  stateHijackFindings: Array<{
    programId: string;
    programName: string;
    accountLabel: string;
  ...
  /**
   * Phase 15 — 'Atomic Execution' Exploit Monitor. TRUE when
   * simulateTransaction detected a transaction that bundles a swap
   * instruction with one or more authority-modifying instructions
   * (SetAuthority, Approve, UpdateMetadataAccount, etc.) in a single
   * atomic execution unit. Forces globalRiskScore = 100 (CRITICAL).
   *
   * DB column: scan_history.is_atomic_exploit BOOLEAN NOT NULL DEFAULT FALSE
   */
  is_atomic_exploit: boolean;

  /**
   * Human-readable summary of the atomic exploit finding.
   * Lists the full instruction tree from the simulation response
   * (swap + authorization instructions).
   *
   * DB column: scan_history.atomic_exploit_details TEXT
   */
  atomic_exploit_details: string | null;

  /**
   * Full instruction list extracted from the simulateTransaction response,
   * including all inner CPI invocations. isAuthorizationRelated instructions
   * are highlighted in red in the Simulation Analysis panel.
   */
  atomicExploitInstructions: import("@/services/analysis/transactionSimulator").SimulatedInstruction[];

  /**
   * Phase 10: Developer History — full result from the cross-token reputation lookup.
   * Null when Supabase is unavailable or the developer wallet is unknown.
   * The classification field drives risk floors in the scoring engine.
   * Use this to display the "Developer History" panel in the Risk Breakdown UI.
   */
  developerHistory: import("@/services/analysis/developerHistory").DeveloperHistoryResult | null;

  /**
   * Phase 16 — 'Rent-Exemption & Account Eviction' Detector.
   * TRUE when at least one account in the token's recent transactions has a
   * lamport balance below getMinimumBalanceForRentExemption(dataLength).
   *
   * DB column: scan_history.has_non_rent_exempt_accounts BOOLEAN NOT NULL DEFAULT FALSE
   */
  is_non_rent_exempt_accounts: boolean;

  /**
   * Structured list of rent-exempt violations. Each entry contains the account
   * address, current lamports, required minimum, and deficit.
   * Used by the Economic Security panel in the Risk Synthesis UI to display
   * the Required vs. Current balance for each at-risk account.
   */
  rentExemptViolations: import("@/services/analysis/rentMonitor").RentExemptViolation[];

  /**
   * Phase 17 — 'Integer Overflow / Underflow' Monitor.
   * TRUE when heuristic analysis of the program's on-chain data detected
   * unchecked arithmetic operations (BPF ADD/SUB/MUL/DIV without bounds-check
   * guards, or Rust unsafe-math byte signatures).
   *
   * When true, scan-core applies a +40 point penalty to globalRiskScore and
   * renders a 'Critical' alert in the UI redFlags list.
   */
  is_overflow_vulnerable: boolean;

  /**
   * Full result from the Integer Overflow / Underflow Monitor (v2 — Context-Aware).
   * Contains the three-phase analysis: static findings, simulation results, and
   * contextual reputation cross-reference. Null when program bytecode was unavailable.
   */
  overflowMonitorResult: import("@/services/analysis/overflowMonitor").ArithmeticSafetyResult | null;

  /**
   * True only when Phase B behavioral simulation confirmed the overflow exploit
   * (simulation succeeded silently without reverting). Differs from is_overflow_vulnerable
   * which is now an alias for this field.
   */
  is_overflow_confirmed_exploit: boolean;

  /**
   * The final alert classification from Phase 17 v2 analysis.
   * One of: "DANGER: Intentional Backdoor Detected" | "CONFIRMED EXPLOIT VECTOR" |
   * "Technical Debt: Audit Required" | "Preliminary Risk" | "Code Style Warning" |
   * "Status: Under Review" | "Safe"
   */
  overflow_alert_tier: string;
  ...
  /**
   * DexScreener pair (pool) address for this token.
   * Passed to enqueueWalletCollection so the worker can paginate
   * pool signatures and extract buyer/seller wallets.
   * null when DexScreener has no pair for this token yet.
   */
...
export function scanToken(address: string): ScanResult {
  ...
  // -- 'Transaction Bloat & Re-routing' Monitor (mock) -----------------------
  // Deterministic CPI-depth roll so the UI can demonstrate the visual warning.
  // Distribution: ~55% depth 1-2 (clean), ~30% depth 3 (obfuscated warning),
  // ~15% depth 4 (Extreme Obfuscation / Critical Risk).
  const cpiRoll = r();
  const cpiDepth = cpiRoll < 0.55 ? (cpiRoll < 0.25 ? 1 : 2)
    : cpiRoll < 0.85 ? 3
    : 4;
  const is_path_obfuscated = cpiDepth >= 3;

  // Apply scoring penalty: deep CPI nesting = +25 risk; extreme = floor at 90.
  if (is_path_obfuscated) {
  ...
  // 'Transaction Bloat & Re-routing' Monitor — surface obfuscated-path warning.
  if (cpiDepth >= 4) {
    redFlags.push({
      ...
      detail:
        "Transaction nesting depth reached the Solana protocol maximum (4). " +
  ...
  return {
    ...
    cpi_risk_details: null,
    cpiTrustedProgramIds: [],
    cpiSuspiciousProgramIds: [],

    is_state_hijacked: false,
    state_hijack_details: null,
    stateHijackFindings: [],

    // Phase 15 — 'Atomic Execution' Exploit Monitor.
    // Mock never flags is_atomic_exploit; only the live simulateTransaction
    // pipeline can detect a real swap+backdoor bundle.
    is_atomic_exploit: false,
    atomic_exploit_details: null,
    atomicExploitInstructions: [],

    // Phase 16 — 'Rent-Exemption & Account Eviction' Detector.
    // Mock always returns clean — rent checks require live RPC calls.
    is_non_rent_exempt_accounts: false,
    rentExemptViolations: [],

    // Phase 17 — Integer Overflow / Underflow Monitor (v2 — Context-Aware).
    // Mock always returns "Status: Under Review" — bytecode analysis requires live RPC.
    is_overflow_vulnerable: false,
    is_overflow_confirmed_exploit: false,
    overflow_alert_tier: "Status: Under Review",
    overflowMonitorResult: null,
