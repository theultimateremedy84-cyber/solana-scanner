...
export interface RiskPhaseBreakdown {
   ...
   *
   * score 0   = no unchecked arithmetic detected (safe math primitives used).
   * score 100 = unchecked arithmetic found (CRITICAL — +40 pts penalty).
   *
   * Heuristic two-phase analysis:
   *   Phase 1: BPF opcode scan — flags ADD/SUB/MUL/DIV without a guard branch.
   *   Phase 2: Rust/Anchor pattern matching — looks for checked_* / saturating_*
   *            vs overflow-checks=false / panic_nounwind signatures.
   */
  overflowMonitor?: {
    score: number;
    label: string;
    /** True when unchecked arithmetic was detected in the program bytecode. */
    ...
    /** Analysis confidence level based on data quantity. */
...
export interface ScanResult {
  ...
  /**
   * Full result from the Integer Overflow / Underflow Monitor.
   * Contains the detailed findings list, unsafe/safe instruction counts,
   * risk level, and analysis confidence. Null when the program bytecode
