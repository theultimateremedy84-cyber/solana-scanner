  ...
  .handler(async ({ data }) => {
    ...
    if (supabaseUrl && supabaseKey) {
      try {
        ...
        url.searchParams.set("developer_wallet", `eq.${data.walletAddress}`);
        url.searchParams.set(
          "select",
          "token_address,token_name,token_symbol,risk_score,risk_level,honey_pot_status,lp_status,market_cap,liquidity,image_url,scanned_at,developer_classification",
        );
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (Array.isArray(json)) dbRows = json;
          }
        } catch {
          clearTimeout(timeout);
        }
    ...
    if (supabaseUrl && supabaseKey) {
      try {
        ...
        url.searchParams.set(
          ...
          "in.(Confirmed Scammer,Serial Offender)",
        );
        url.searchParams.set(
          "select",
          "developer_wallet,developer_classification,token_address,token_name,token_symbol,risk_score,risk_level,scanned_at",
        );
        url.searchParams.set("order", "scanned_at.desc");
