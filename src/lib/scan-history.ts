...
export interface ScanHistoryRow {
  ...
  /**
   * Phase 16 — 'Rent-Exemption & Account Eviction' Detector flag.
   * TRUE when at least one account identified in the token's recent
   * transactions has a lamport balance below its rent-exempt minimum.
   * Accounts below this threshold are susceptible to eviction and
   * state hijacking via account resurrection.
   *
   * DB column: BOOLEAN NOT NULL DEFAULT FALSE
   *   (see supabase/migrations/20260622000000_rent_exempt_monitor.sql)
   */
...
/**
 * AUDIT FIX (finding #4): previously this inserted directly into scan_history
 * using the anon-key browser client, and the table's RLS allowed open,
 * unvalidated INSERT from anon/authenticated — anyone with the anon key
 * could write arbitrary/spoofed risk data. It now POSTs to a server route
 * (/api/scan-history) that validates every field before writing with the
 * service role key. See src/lib/api/scan-history-handler.ts and
 * supabase/migrations/20260709000003_scan_history_hardening.sql, which
 * revokes direct anon/authenticated INSERT on this table — deploy both
 * together.
 */
export async function recordScan(result: ScanResult): Promise<void> {
  const row = {
    ...
    // 'Rent-Exemption & Account Eviction' Detector (Phase 16).
    has_non_rent_exempt_accounts: result.is_non_rent_exempt_accounts,
  };
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[recordScan]", res.status, body);
    }
  } catch (err) {
    console.error("[recordScan]", err instanceof Error ? err.message : String(err));
  }
}
