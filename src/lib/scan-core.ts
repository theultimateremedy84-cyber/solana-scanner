...
export function buildScanResult(input: RawInputs): ScanResult {
  ...
  //   cluster-rugged token: +18 each   <-- NEW
  ...
  // Keep `devTrustScore` populated for backwards compatibility with any
  // downstream consumer, but it is no longer surfaced as a headline
  ...
  // If the upgrade / mint / freeze authority is held by a plain wallet
  // (overallClassification === "Unsafe/Upgradeable"), the token can never be
  // rated LOW trust. We raise globalRiskScore to at least 20 (start of MEDIUM)
  // and flag the authority result so the UI can explain the floor.
  ...
  // If the LP is unlocked (not burned, not locked) the developer can drain
  // the pool at any moment. This is a critical exit-risk and must never
  // present as LOW (green). Floor the score firmly into HIGH territory
  // (HIGH starts at 40 in `level()`; 55 leaves room above MEDIUM and
  // signals red in the UI without forcing EXTREME).
  if (lpStatus === "Unlocked") {
    globalRiskScore = Math.max(globalRiskScore, 55);
  }

  const riskScore = globalRiskScore; // keep riskScore in sync for the existing UI
  ...
  // Include cluster-rugged history so the serial-scammer label reflects
  // the same evidence the UI shows under Wallet Cluster.
