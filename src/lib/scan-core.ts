...
interface RawInputs {
  ...
  trades?: Trade[] | null;     // recent swap history for manipulation engine
  resolvedFromPair?: boolean;
  originalInput?: string;
  /** Off-chain intelligence (social sentiment + website audit + intent score). */
...
export function buildScanResult(input: RawInputs): ScanResult {
  const { address, parsed, largest, supplyResp, rug, asset, pair, sniper, honey, trades, resolvedFromPair, originalInput, offChain } = input;
  ...
  // --- Advanced wash-trading / manipulation detection ---
  // Replaces the naive vol/liquidity ratio with a 4-layer engine when real
  // trade history is available. Falls back to the ratio heuristic otherwise.
            ...
            ? [
                {
                  ...
                  description: `24h volume is ${ratio.toFixed(2)}x the available liquidity. The fallback model flags ratios above 4x as suspicious and above 8x as likely manipulated when trade-level history is unavailable.`,
                  weight: Math.round(100 - volumeIntegrity),
                  evidence: {
                    volume24h,
                    liquidity,
                    volumeLiquidityRatio: Number(ratio.toFixed(2)),
                    suspiciousThreshold: "4x",
                    likelyManipulatedThreshold: "8x",
                  },
                },
              ]
            : [],
  ...
  // When the engine ran, let its anomaly score drive Volume Integrity so the
  // category breakdown reflects the richer analysis.
  ...
  //
  // When off-chain data is present, the Developer Reputation category uses
  // the unified dev score (which already blends on-chain trust with the
  // intent penalty). Without it we fall back to the pure on-chain devScore.
  //
  // Rule: "Safe on-chain code does not override dangerous off-chain behavior."
  // A clean on-chain result paired with a bad intent verdict must still
  // surface as high risk — the floor applied below enforces this.
  ...
  const offChainDevNotes: string = offChainAvailable
    ? `Off-chain intent: ${offChain!.intentVerdict.replace(/_/g, " ")} (${offChain!.intentScore}/100). ` +
      `Website grade: ${offChain!.websiteAuthenticityGrade}. ` +
      `Hype: ${offChain!.hypeVerdict}.`
    : "";
  ...
  // --- Off-chain floor rule ---
  // "Safe on-chain code does not override dangerous off-chain behavior."
  // If off-chain intent is suspicious or worse, the final risk score must
  // be at least as high as the intent score, regardless of on-chain signals.
  if (offChainAvailable && offChain!.intentVerdict !== "genuine" && offChain!.intentVerdict !== "unavailable") {
    const intentFloor =
      offChain!.intentVerdict === "likely_scam"
        ? 60
        : offChain!.intentVerdict === "suspicious"
          ? 40
          : 0;
    riskScore = Math.max(riskScore, intentFloor);
  }
  ...
  if (washTrading.available && washTrading.anomalyScore >= 35) {
    const sev =
      washTrading.verdict === "manipulated"
        ? "critical"
        : washTrading.verdict === "likely_manipulated"
          ? "high"
          : "warn";
    const topPatterns = washTrading.patterns.slice(0, 3).map((pat) => pat.label);
    redFlags.push({
      id: "wash",
      severity: sev as RedFlag["severity"],
      title: `Wash trading / manipulation: ${washTrading.verdict.replace(/_/g, " ")}`,
      detail: `Anomaly score ${washTrading.anomalyScore}/100 across ${washTrading.tradesAnalyzed} analysed swaps.${
        topPatterns.length ? ` Patterns: ${topPatterns.join(", ")}.` : ""
      }`,
    });
    ...
    redFlags.push({
      ...
      detail: `Volume/liquidity ratio ${ratio.toFixed(1)} is abnormally high (trade history unavailable for deep analysis).`,
    });
  }
  ...
  if (offChainAvailable) {
    // Website authenticity flags (forward the most severe ones to the main red-flag list)
    for (const wf of offChain!.websiteFlags.filter(
      (f) => f.severity === "critical" || f.severity === "high",
    )) {
      redFlags.push({
        id: `offchain-web-${wf.id}`,
        severity: wf.severity,
        title: `Website: ${wf.label}`,
        detail: wf.detail,
      });
    }

    // Intent verdict flags
    if (offChain!.intentVerdict === "likely_scam") {
      redFlags.push({
        id: "offchain-intent",
        severity: "critical",
        title: "Off-chain intent: likely scam",
      ...
      redFlags.push({
        id: "offchain-intent",
        severity: "high",
        title: "Off-chain intent: suspicious",
    ...
    // Spam-heavy social signal
    if (offChain!.hypeVerdict === "spam_heavy") {
      redFlags.push({
        id: "offchain-hype",
        severity: "warn",
        title: "Social channels dominated by spam",
