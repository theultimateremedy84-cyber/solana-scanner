...
interface RawInputs {
  ...
  /** Authority monitor result from runAuthorityMonitor(). */
  authorityMonitor?: AuthorityMonitorResult | null;
}
...
export function buildScanResult(input: RawInputs): ScanResult {
  const {
    ...
    sniper, honey, trades, resolvedFromPair, originalInput, offChain,
    authorityMonitor,
  } = input;
  ...
  const devTrustScore = clamp(100 - verifiedScams * 18 - reportedScams * 6 - (mintActive ? 10 : 0), 0, 100);
  ...
  // Phase groupings and weights are chosen so they sum to 1.0.
  ...
  // Rule: a confirmed honeypot or a likely-scam off-chain verdict locks the
  // minimum risk at 70 regardless of any other metric.
  ...
  // Applied after the base to ensure behavioral manipulation can push a
  // "safe-code" token into the danger zone.
  ...
  // -----------------------------------------------------------------------
  // Step 7: UPGRADEABLE AUTHORITY FLOOR
  //
  // Rule: when any authority (mint, freeze, or metadata update) is held by a
  // single wallet — "Unsafe/Upgradeable" — the token cannot receive a LOW
  // risk rating, because the key-holder can modify critical parameters
  // unilaterally at any time.
  //
  // Enforcement: globalRiskScore is floored at 20 (the bottom of the MEDIUM
  // band), preventing a LOW (<20) verdict regardless of every other metric.
  // -----------------------------------------------------------------------
  const isUpgradeable =
    (authorityMonitor?.overallClassification ?? "Unsafe/Upgradeable") === "Unsafe/Upgradeable";
  const upgradeableFloorApplied = isUpgradeable && globalRiskScore < 20;
  if (isUpgradeable) {
    globalRiskScore = Math.max(globalRiskScore, 20);
  }

  // Finalise score and level.
  const riskScore = globalRiskScore;
  const riskLevel = level(globalRiskScore);

  // -----------------------------------------------------------------------
  // CONFIDENCE METRIC
  // Based on the quantity and quality of data points available for this scan.
  // -----------------------------------------------------------------------
  ...
  const verdictSummary: string = (() => {
    ...
    if (isUpgradeable && authorityMonitor?.available)
      return `Contract is upgradeable — ${authorityMonitor.upgradeAuthority.address ? "metadata update" : "mint or freeze"} authority is held by a single wallet and can be changed without warning.`;
    if (mintActive)
  ...
  const riskBreakdown: RiskPhaseBreakdown = {
    onChainCode: {
      ...
      driver: isConfirmedHoneypot
        ...
        : isUpgradeable && authorityMonitor?.available
          ? `Upgradeable: ${authorityMonitor.overallClassification}`
          : mintActive
  ...
  // Authority monitor red flags (placed first — most critical for upgradeable tokens)
  if (authorityMonitor?.available) {
    if (isUpgradeable) {
      const upgradeableDetails: string[] = [];
      if (authorityMonitor.upgradeAuthority.classification === "Unsafe/Upgradeable" && authorityMonitor.upgradeAuthority.address)
        upgradeableDetails.push(`Metadata update: ${authorityMonitor.upgradeAuthority.reason}`);
      if (authorityMonitor.mintAuthority.classification === "Unsafe/Upgradeable" && authorityMonitor.mintAuthority.address)
        upgradeableDetails.push(`Mint: ${authorityMonitor.mintAuthority.reason}`);
      if (authorityMonitor.freezeAuthority.classification === "Unsafe/Upgradeable" && authorityMonitor.freezeAuthority.address)
        upgradeableDetails.push(`Freeze: ${authorityMonitor.freezeAuthority.reason}`);

      redFlags.push({
        id: "authority-upgradeable",
        severity: "high",
        title: "Contract is upgradeable — single-wallet authority",
        detail: upgradeableDetails.length
          ? upgradeableDetails.join(" | ")
          : "One or more authorities are controlled by a single wallet and can be changed unilaterally. Risk score floored at MEDIUM.",
      });

      if (upgradeableFloorApplied) {
        redFlags.push({
          id: "authority-floor",
          severity: "warn",
          title: "Risk floor applied: upgradeable contract",
          detail: "Global Risk Score raised to MEDIUM minimum (≥ 20) because this token has at least one single-wallet-controlled authority.",
        });
      }
    } else if (authorityMonitor.overallClassification === "Medium Risk") {
      redFlags.push({
        id: "authority-multisig",
        severity: "warn",
        title: "Authority held by multisig",
        detail: "One or more authorities are controlled by a multisig — changes require multiple signers, reducing unilateral risk.",
      });
    }
  } else if (!authorityMonitor) {
    // Authority monitor not run — default to flagging active authorities
    if (mintActive)
  ...
  if (offChainAvailable) {
    ...
    if (offChain!.hypeVerdict === "spam_heavy") {
      redFlags.push({
        ...
        detail: `Hype score ${offChain!.hypeScore}/100 — majority of social mentions are low-quality or artificial.`,
      });
    }
  }

  // -----------------------------------------------------------------------
  // AUTHORITY MONITOR — attach upgradeableRiskFloorApplied flag
  // -----------------------------------------------------------------------
  const finalAuthorityMonitor: AuthorityMonitorResult = authorityMonitor
    ? { ...authorityMonitor, upgradeableRiskFloorApplied: upgradeableFloorApplied }
    : {
        available: false,
        upgradeAuthority: {
          address: null,
          ...
          isMultisig: false,
          reason: "Authority monitor did not run — defaulting to conservative classification.",
        },
        mintAuthority: {
          address: mintAuthorityRaw ?? null,
          classification: mintActive ? "Unsafe/Upgradeable" : "Immutable/Safe",
          isMultisig: false,
          reason: mintActive
            ? "Mint authority is active (RPC data only, multisig check skipped)."
            : "Mint authority is revoked.",
        },
        freezeAuthority: {
          address: freezeAuthorityRaw ?? null,
          classification: freezeActive ? "Unsafe/Upgradeable" : "Immutable/Safe",
          isMultisig: false,
          reason: freezeActive
            ? "Freeze authority is active (RPC data only, multisig check skipped)."
            : "Freeze authority is revoked.",
        },
        overallClassification: mintActive || freezeActive ? "Unsafe/Upgradeable" : "Immutable/Safe",
        upgradeableRiskFloorApplied: upgradeableFloorApplied,
      };

  // -----------------------------------------------------------------------
  // ASSEMBLE FINAL RESULT
  // -----------------------------------------------------------------------
  const devScoreDisplay = clamp(devTrustScore);
  const serialProbability: ScanResult["serialScammerProbability"] =
    verifiedScams >= 3 ? "Confirmed Pattern"
    : verifiedScams >= 2 ? "High"
    : verifiedScams === 1 ? "Medium"
    : "Low";
  const dna = clamp(
    riskScore * 0.35 +
    (100 - devTrustScore) * 0.4 +
    verifiedScams * 8 +
    (lpStatus === "Unlocked" ? 12 : 0),
  );
  ...
  return {
    ...
    volumeIntegrity: effectiveVolumeIntegrity,
    washTrading,
    sniperPct,
    sniperWallets,
    sniperRisk,

    devTrustScore: devScoreDisplay,
    ...
    serialScammerProbability: serialProbability,
    scammerDnaScore: Math.round(dna),

    clusterId: "CL-" + address.slice(-4).toUpperCase(),
    clusterWallets: 0,
    clusterTokens: creatorTokens,
    ...
    authorityMonitor: finalAuthorityMonitor,
