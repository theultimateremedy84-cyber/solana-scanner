...
export function buildScanResult(input: RawInputs): ScanResult {
  ...
  // -----------------------------------------------------------------------
  // Step 18: 'INTEGER OVERFLOW / UNDERFLOW' MONITOR (Phase 17 v2)
  //
  // Three-phase context-aware analysis (see overflowMonitor.ts):
  //   Phase A: Static analysis — "Preliminary Risk" flag (is_preliminary_risk).
  //   Phase B: Behavioral simulation — "CONFIRMED EXPLOIT VECTOR" (is_confirmed_exploit).
  //   Phase C: Developer reputation — "DANGER: Intentional Backdoor Detected".
  //
  // Scoring is now tiered based on the highest confirmed alert tier:
  //   - Preliminary Risk only (simulation safe/inconclusive) → +10 pts additive.
  //   - Confirmed Exploit Vector (simulation failed) → +40 pts additive.
  //   - DANGER (confirmed exploit + serial scammer) → +40 pts (same penalty, flagged separately).
  //   - Code Style Warning (simulation reverted → false positive) → no penalty.
  //
  // This step is additive and lower priority than the force-100 overrides.
  // -----------------------------------------------------------------------
  const isOverflowPreliminaryRisk: boolean = !!overflowMonitor?.is_preliminary_risk;
  const isOverflowVulnerable: boolean = !!overflowMonitor?.is_confirmed_exploit;
  const overflowAlertTier: string = overflowMonitor?.alert_tier ?? "Safe";

  if (isOverflowVulnerable) {
    globalRiskScore = clamp(globalRiskScore + 40);
  } else if (
    isOverflowPreliminaryRisk &&
    overflowAlertTier !== "Code Style Warning" &&
    overflowAlertTier !== "Safe"
  ) {
    globalRiskScore = clamp(globalRiskScore + 10);
  }
  ...
  const riskBreakdown: RiskPhaseBreakdown = {
    ...
    // Tiered scoring: 0=Safe, 50=Preliminary, 75=CodeStyleWarning, 100=ConfirmedExploit.
    ...(overflowMonitor != null
      ? {
          overflowMonitor: {
            score: isOverflowVulnerable
              ? 100
              : overflowAlertTier === "Status: Under Review"
                ? 50
                : overflowAlertTier === "Preliminary Risk"
                  ? 50
                  : overflowAlertTier === "Code Style Warning"
                    ? 25
                    : 0,
            label: "Integer Overflow / Underflow Monitor",
            is_preliminary_risk: isOverflowPreliminaryRisk,
            is_confirmed_exploit: isOverflowVulnerable,
            is_vulnerable: isOverflowVulnerable,
            alert_tier: overflowAlertTier,
            verification_method: overflowMonitor.verification_method,
            ...
            simulation: overflowMonitor.simulation,
            driver: (() => {
              switch (overflowAlertTier) {
                case "DANGER: Intentional Backdoor Detected":
                  return `🚨 DANGER: Intentional Backdoor — confirmed exploit + serial scammer developer (+40 pts)`;
                case "CONFIRMED EXPLOIT VECTOR":
                  return `🔢 CONFIRMED EXPLOIT VECTOR — ${overflowMonitor.unsafe_instruction_count} unchecked instruction(s), simulation verified (+40 pts)`;
                case "Technical Debt: Audit Required":
                  return `⚠️ Technical Debt — exploit confirmed but verified audit on file (+40 pts)`;
                case "Preliminary Risk":
                  return `⚠️ Preliminary Risk — ${overflowMonitor.unsafe_instruction_count} unchecked instruction(s), simulation inconclusive (+10 pts)`;
                case "Code Style Warning":
                  return `ℹ️ Code Style Warning — static flags found but simulation confirmed safe runtime guards (no penalty)`;
                case "Status: Under Review":
                  return `🔍 Status: Under Review — bytecode unavailable or simulation inconclusive (+10 pts)`;
                default:
                  return `✅ No unchecked arithmetic detected — safe math primitives in use`;
              }
            })(),
          },
        }
  ...
  // Phase 17: 'Integer Overflow / Underflow' Monitor — tiered red flags.
  if (overflowMonitor != null && overflowAlertTier !== "Safe") {
    if (overflowAlertTier === "DANGER: Intentional Backdoor Detected") {
      redFlags.push({
        id: "overflow-underflow",
        severity: "critical",
        title: "🚨 DANGER: Integer Overflow — Intentional Backdoor Detected",
        detail:
          `Behavioral simulation confirmed ${overflowMonitor.unsafe_instruction_count} unchecked arithmetic path(s) ` +
          `that allow silent overflow/underflow without reverting, COMBINED with a ` +
          `Confirmed Scammer / Serial Offender developer classification. ` +
          `The combination of a verified exploit vector and malicious developer history ` +
          `strongly indicates a deliberate backdoor. ` +
          `Verification method: ${overflowMonitor.verification_method}. ` +
          `Risk penalty: +40 pts added to global score.`,
      });
    } else if (overflowAlertTier === "CONFIRMED EXPLOIT VECTOR") {
      redFlags.push({
        id: "overflow-underflow",
        severity: "critical",
        title: "🔢 CONFIRMED EXPLOIT VECTOR: Integer Overflow/Underflow",
        detail:
          `Behavioral simulation verified that ${overflowMonitor.unsafe_instruction_count} unchecked arithmetic ` +
          `instruction(s) allow integer overflow/underflow without triggering a runtime revert. ` +
          `An attacker can exploit this to manipulate token supply, balance accounting, or fee ` +
          `calculations — classic inflation attack and balance underflow drain vectors. ` +
          `Simulation: u64::MAX+1 and u64::MIN-1 edge cases succeeded silently. ` +
          `Verification method: ${overflowMonitor.verification_method}. ` +
          `Risk penalty: +40 pts added to global score.`,
      });
    } else if (overflowAlertTier === "Technical Debt: Audit Required") {
      redFlags.push({
        id: "overflow-underflow",
        severity: "high",
        title: "⚠️ Technical Debt: Integer Overflow — Audit Required",
        detail:
          `Behavioral simulation confirmed an overflow exploit vector (${overflowMonitor.unsafe_instruction_count} unchecked ` +
          `arithmetic instruction(s)), but the project has a verified security audit on file. ` +
          `This may represent known/accepted technical debt rather than malicious intent, ` +
          `but should be investigated before investing. ` +
          `Verification method: ${overflowMonitor.verification_method}. ` +
          `Risk penalty: +40 pts added to global score.`,
      });
    } else if (overflowAlertTier === "Preliminary Risk") {
      redFlags.push({
        id: "overflow-underflow",
        severity: "warn",
        title: "⚠️ Preliminary Risk: Unchecked Arithmetic Detected",
        detail:
          `Static analysis found ${overflowMonitor.unsafe_instruction_count} unchecked arithmetic instruction(s) ` +
          `in the program bytecode, but behavioral simulation was inconclusive. ` +
          `This is a Heuristic Warning — further investigation is recommended. ` +
          `Verification method: ${overflowMonitor.verification_method}. ` +
          `Risk penalty: +10 pts added to global score.`,
      });
    } else if (overflowAlertTier === "Code Style Warning") {
      redFlags.push({
        id: "overflow-underflow",
        severity: "info",
        title: "ℹ️ Code Style: Unchecked Arithmetic (Simulation Confirmed Safe)",
        detail:
          `Static analysis flagged ${overflowMonitor.unsafe_instruction_count} arithmetic instruction(s) without visible ` +
          `guard branches, but behavioral simulation confirmed the program has runtime overflow ` +
          `guards — overflow inputs would cause a revert. The unchecked opcodes appear to be ` +
          `in non-critical paths. No risk penalty applied. ` +
          `Verification method: ${overflowMonitor.verification_method}.`,
      });
    } else if (overflowAlertTier === "Status: Under Review") {
      redFlags.push({
        id: "overflow-underflow",
        severity: "warn",
        title: "🔍 Status: Under Review — Integer Arithmetic Analysis",
        detail:
          `Program bytecode analysis returned inconclusive results. ` +
          `Bytecode may have been unavailable or the simulation could not determine ` +
          `whether overflow paths are exploitable. Raw data is available in the ` +
          `Overflow Monitor panel for advanced review. ` +
          `Risk penalty: +10 pts added to global score as a precautionary measure.`,
      });
    }
  }
  ...
  return {
    ...
    is_overflow_confirmed_exploit: isOverflowVulnerable,
    overflow_alert_tier: overflowAlertTier,
