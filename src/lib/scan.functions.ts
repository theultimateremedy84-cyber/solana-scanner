 ...
 * `analyzeArithmeticSafety` for heuristic BPF opcode + Rust pattern analysis.
 *
 * Returns a safe fallback result (is_vulnerable: false, confidence: "low") if
 * the account data is unavailable or cannot be decoded.
 */
async function fetchProgramBytecodeAnalysis(mintAddress: string): Promise<ArithmeticSafetyResult> {
  ...
  try {
    ...
    return analyzeArithmeticSafety(bytes);
  } catch (err) {
...
export const scanTokenLive = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ScanResult> => {
    ...
    const [authorityTransitioned, accountResized, metadataHijacked, pathObfuscated, cpiManipulation, stateHijack, developerHistory, atomicExploitCached, atomicExploitLive, rentExempt, overflowMonitor] = await Promise.all([
      ...
      // Phase 17: 'Integer Overflow / Underflow' Monitor — heuristic BPF scan.
