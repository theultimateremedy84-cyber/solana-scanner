...
export const scanTokenLive = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ScanResult> => {
    ...
    // 7. Post-launch checks — fetch both flags from Supabase in parallel so
    //    even if PostLaunchWatcher fired while the user was away, the re-scan
    //    immediately reflects Critical Risk status. The DB lookups are always
    //    live (no cache) so a TRUE written by the watcher is never stale.
    const [authorityTransitioned, accountResized] = await Promise.all([
    ...
    console.log(`[scanToken] Final globalRiskScore for ${address}: ${result.globalRiskScore} (is_account_resized=${result.is_account_resized})`);
