...
async function fetchRentExemptionStatus(mintAddress: string): Promise<RentMonitorResult> {
  ...
  try {
    ...
    // Build a map from address → { lamports, dataLength }.
    const accountDataMap = new Map<string, { lamports: number; dataLength: number }>();
    ...
    for (const { key, value } of infos) {
      ...
      accountDataMap.set(key, { lamports, dataLength });
    ...
    for (const [address, { lamports, dataLength }] of accountDataMap) {
      const requiredMinimum = minBalanceMap.get(dataLength);
      if (!requiredMinimum) continue; // skip if we couldn't fetch minimum
      accountRentInfos.push({ address, lamports, dataLength, requiredMinimum });
    }
