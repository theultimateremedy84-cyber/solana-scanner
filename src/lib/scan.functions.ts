...
import { runAuthorityMonitor } from "@/services/analysis/authorityMonitor";
...
export const scanTokenLive = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ScanResult> => {
    ...
    // 4. Helius — DAS asset (token metadata: name/symbol/image + authorities)
    let asset: any = null;
    if (HELIUS_KEY()) {
    ...
    // 7. Authority Monitor — classify mint, freeze, and metadata update authorities
    // The Helius DAS asset.authorities array carries the metadata update authority.
    // Scope "full" or "metadata" identifies the update authority entry.
    const upgradeAuthAddress: string | null = Array.isArray(asset?.authorities)
      ? (asset.authorities.find(
          (a: any) =>
            Array.isArray(a?.scopes) &&
            (a.scopes.includes("full") || a.scopes.includes("metadata")),
        )?.address ?? null)
      : null;

    const authorityMonitor = await runAuthorityMonitor({
      mintAuthorityAddress: parsed?.mintAuthority ?? null,
      freezeAuthorityAddress: parsed?.freezeAuthority ?? null,
      upgradeAuthorityAddress: upgradeAuthAddress,
      rpcUrl: RPC_URL(),
    });
