...
export const scanTokenLive = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ScanResult> => {
    ...
    // 7. Post-launch checks — fetch all three flags from Supabase in parallel
    //    so even if PostLaunchWatcher fired while the user was away the re-scan
    //    immediately reflects Critical Risk status. DB lookups are always live
    //    (no cache) so a TRUE written by the watcher is never stale.
    const [authorityTransitioned, accountResized, metadataHijacked] = await Promise.all([
