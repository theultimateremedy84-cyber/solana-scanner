...
import { isHeliusBudgetExhausted } from "./tx-reconstructor";
 ...
 */
async function findHollowPairs(
...
async function tick(): Promise<void> {
  ...
  // P3-C: skip the entire tick when the shared Helius credit budget is
  // exhausted. Without this guard the scheduler would attempt
  // TOKENS_PER_RUN × WALLETS_PER_TOKEN enrichments, each of which would
  // immediately short-circuit in _consumeHC — wasting the tick and
  // flooding logs with redundant budget-exhausted warnings.
  if (isHeliusBudgetExhausted()) {
    console.warn(`${LOG} Helius budget exhausted — skipping tick to preserve remaining credits`);
    return;
  }
  ...
  try {
    ...
    // Sort tokens by hollow-wallet count (highest first); take TOKENS_PER_RUN
    const topTokens = Array.from(tokenMap.entries())
      .sort((a, b) => b[1].length - a[1].length)
      .slice(0, TOKENS_PER_RUN);
