...
const TOKENS_PER_RUN = 10;  // 10 tokens per tick — unchanged
    ...
    for (const [tokenAddress, wallets] of topTokens) {
      ...
      try {
        ...
        const result = await enrichWalletsForToken({
          ...
          delayMs:         300, // BACKLOG_MODE: reduced from 800ms — budget is unlimited;
                                // Helius 429s are handled by retry logic in tx-reconstructor.
                                // Lower delay = higher throughput without dropping data quality.
        });
