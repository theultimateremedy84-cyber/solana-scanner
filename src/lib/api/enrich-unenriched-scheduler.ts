...
const INTERVAL_MS = 5 * 60 * 1_000; // 5 minutes (reduced from 30 — expedited backlog clearance 2026-07-09)
// NOTE: the `running` guard prevents concurrent ticks. If a tick takes longer
// than 5 minutes (likely with WALLETS_PER_TOKEN=75), the next interval fires
// but is immediately skipped. Effective cadence = tick duration, not interval.
// Setting interval short ensures we start the next tick as soon as possible
// after the previous one finishes rather than waiting the full 30 minutes.
...
const TOKENS_PER_RUN = 10;  // raised from 5 — audit 2026-07-08: 38k hollow pair backlog; no Helius budget constraint
...
const WALLETS_PER_TOKEN = 75; // raised from 25 — expedited backlog clearance 2026-07-09
// At 75 wallets × 10 tokens × ~1s/wallet ≈ 12-15 min per tick.
// 23,534 hollow wallets ÷ 750 per tick = ~32 ticks ≈ 7 hours to clear backlog
// (vs ~47 hours at the previous 25 wallets × 30-minute interval).
// No Helius budget constraint — safe to raise.
...
export async function findHollowPairs(
  ...
  // 5× headroom over max consumption (TOKENS_PER_RUN × WALLETS_PER_TOKEN)
  // ensures the anti-starvation selection always has enough candidate rows.
  const RPC_LIMIT = TOKENS_PER_RUN * WALLETS_PER_TOKEN * 5;
