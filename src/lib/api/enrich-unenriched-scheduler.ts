...
const TOKENS_PER_RUN = 5;  // reverted from 10 — Fix 6 doubled this causing Helius credit exhaustion
 ...
 */
const LONG_TAIL_SLOTS = 2; // reduced from 3 to match new TOKENS_PER_RUN=5

/** Max wallets to enrich per token per tick (respects Helius rate limits). */
const WALLETS_PER_TOKEN = 10; // reverted from 20 — Fix 6 doubled this causing Helius credit exhaustion
...
export async function findHollowPairs(
  ...
  const { data, error } = await (sb as any).rpc("find_hollow_pairs");
