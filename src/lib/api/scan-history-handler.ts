...
/** Simple in-memory sliding-window rate limit: one recorded scan per
 * token_address per window, regardless of caller. Resets on redeploy —
 * acceptable here since this only needs to blunt bursts, not be exact. */
const RATE_LIMIT_WINDOW_MS = 30_000;
const lastRecordedAt = new Map<string, number>();

function isRateLimited(tokenAddress: string): boolean {
  const now = Date.now();
  const last = lastRecordedAt.get(tokenAddress);
  if (last !== undefined && now - last < RATE_LIMIT_WINDOW_MS) return true;
  lastRecordedAt.set(tokenAddress, now);
  // Opportunistic cleanup so this map can't grow unbounded over a long
  // process lifetime.
  if (lastRecordedAt.size > 5_000) {
    for (const [key, ts] of lastRecordedAt) {
      if (now - ts > RATE_LIMIT_WINDOW_MS) lastRecordedAt.delete(key);
    }
  }
  return false;
}
...
export async function handleScanHistoryPost(request: Request): Promise<Response> {
  ...
  if (isRateLimited(row.token_address as string)) {
