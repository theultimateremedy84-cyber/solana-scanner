...
// Supabase/PostgREST caps a single REST/JS-client response at 1000 rows by
// default (db-max-rows). Requesting more from find_hollow_pairs() would be
// silently truncated to 1000 anyway, so we request exactly the cap and treat
// a full page as a floor rather than the true remaining count.
const HOLLOW_PAIRS_QUERY_LIMIT = 1000;
...
export async function handleBacklogStatusGet(): Promise<Response> {
  try {
    // NOTE: src/integrations/supabase/types.ts is auto-generated and predates
    // several tables/RPCs used here (wallet_collection_jobs, helius_cu_log,
    // alerts, wallet_sol_transfers, find_hollow_pairs, get_helius_cu_totals,
    // get_helius_cu_top_components) — the underlying tables/functions exist
    // in the migrations and in production, but the generated `Database` type
    // doesn't know about them yet, which would otherwise reject every query
    // below at compile time. Cast to `any` here rather than hand-editing the
    // generated file; regenerate types.ts (`supabase gen types`) to remove
    // this cast once it's back in sync.
    ...
    const [
      ...
      sb.from("wallet_raw_tx_metrics").select("id", { count: "exact", head: true }).eq("data_source", "helius_full_history"),
      sb.rpc("find_hollow_pairs", { p_limit: HOLLOW_PAIRS_QUERY_LIMIT }),
    ...
    const errors = [
      ...
      totalPerformanceRecordsRes.error, totalEnrichedRecordsRes.error, hollowPairsRes.error,
    ...
    const hollowPairs = (hollowPairsRes.data ?? []) as { wallet_address: string; token_address: string }[];
    ...
    return json({
      ...
      enrichment: {
        ...
        hollowPairsPending: hollowPairs.length,
        hollowPairsPendingIsFloor: hollowPairs.length >= HOLLOW_PAIRS_QUERY_LIMIT,
