...
async function runJobBackground(
  ...
  try {
    ...
    await safeUpdateJob(sb, jobId, "failed", { last_error: msg }, startedAt).catch(() => {});
  }
}
  ...
  try {
    ...
    for (const job of giveUp) {
      ...
      await safeUpdateJob(sb, jobId, "failed", {
        last_error: `Exceeded max attempts (${attempts}/${MAX_ATTEMPTS}) — gave up without a recorded per-attempt error. Check collect() / Helius connectivity for this token.`,
      }).catch(() => {});
    }
    ...
    for (const job of retryable) {
      ...
      const { error: stampErr } = await sb
        ...
        .update({
          ...
          attempts:   ((job.attempts as number) ?? 0) + 1,
          ...(timeoutNote ? { last_error: timeoutNote } : {}),
        })
        .eq("id", jobId)
