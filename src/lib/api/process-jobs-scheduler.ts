...
async function tick(): Promise<void> {
  ...
  try {
    ...
    for (const job of giveUp) {
      ...
      // FIX (Issue 5 — giveUp path loses per-attempt history):
      //   The previous code wrote a fresh errors array with only a single
      //   "max_attempts_exceeded" entry, discarding all per-attempt error
      //   objects accumulated by the FIX #5 timeout-note logic.  Instead:
      //   1. Read the existing errors JSONB from the job row (already fetched
      //      by the select("*") above).
      //   2. Append the give-up event to that array — preserving all prior
      //      attempt entries so analysts can trace the failure chain.
      //   3. Surface the most recent prior error in last_error so it is
      //      immediately visible in the Supabase table without parsing JSONB.
      const existingErrors: Array<{ attempt: number; error: string; at: string }> =
        Array.isArray(job.errors) ? (job.errors as Array<{ attempt: number; error: string; at: string }>) : [];
      const latestErr = existingErrors.length > 0 ? existingErrors[existingErrors.length - 1] : null;
      const giveUpMsg = latestErr
        ? `Exceeded max attempts (${attempts}/${MAX_ATTEMPTS}). Last captured error: "${latestErr.error}" at attempt ${latestErr.attempt} (${latestErr.at}).`
        : `Exceeded max attempts (${attempts}/${MAX_ATTEMPTS}) — no per-attempt error recorded. ` +
          `Check Railway logs around each started_at timestamp for this token.`;
      console.error(
        `${LOG} [${jobId}] Exceeded MAX_ATTEMPTS (${attempts}/${MAX_ATTEMPTS}) — marking failed, not retrying.` +
        (latestErr ? ` Last error: "${latestErr.error}"` : ""),
      );
      await safeUpdateJob(sb, jobId, "failed", {
        last_error: giveUpMsg,
        errors: [
          ...existingErrors,
          { attempt: attempts, error: "max_attempts_exceeded", at: new Date().toISOString() },
        ],
      }).catch(() => {});
    }
