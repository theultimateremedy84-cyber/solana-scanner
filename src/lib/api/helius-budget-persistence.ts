...
function setBudgetGlobal(used: number): void {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const g = globalThis as any;
  const daily = parseInt(process.env.HELIUS_DAILY_BUDGET ?? "0", 10) || 0;
  if (!g.__heliusBudget__) {
    ...
    // FIX (P0 #5): use calendar UTC day to match _consumeHC's reset logic.
    g.__heliusBudget__ = {
      budget:      daily,
      used,
    ...
    .upsert(
      {
        date,
        cu_used:      b.used,
        // FIX: persist the configured ceiling and computed headroom so the
        // monitor dashboard shows real utilisation instead of always 0/0.
        // cu_budget comes from HELIUS_DAILY_BUDGET (seeded into globalThis on startup).
        cu_budget:    b.budget,
        // FIX (P2): when HELIUS_DAILY_BUDGET=0 (unlimited mode),
        // Math.max(0, 0 - 0) = 0, which makes the dashboard show "0 CU remaining"
        // — indistinguishable from a completely exhausted budget. Use null instead
        // so the monitor can display "unlimited" rather than "exhausted".
        cu_remaining: b.budget === 0 ? null : Math.max(0, b.budget - b.used),
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        ...
        `Remaining: ${Math.max(0, (parseInt(process.env.HELIUS_DAILY_BUDGET ?? "0", 10) || 0) - data.cu_used)} CUs.`,
      );
    } else {
