...
export interface LeaderboardWallet {
  ...
  // total_buys / total_sells removed — unreliable for fallback-path wallets.
  // Use verified_positions (closed positions with real cost-basis) instead.
  realized_pnl:          number;
...
export const getLeaderboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{ wallets: LeaderboardWallet[]; total: number; error: string | null }> => {
    ...
    // NOTE: total_buys and total_sells are intentionally omitted — they are
    // unreliable (zero for ~40% of fallback-path wallets) and are not displayed
    // in the UI. verified_positions is the correct user-facing trade-count metric.
    const COLS = [
      "wallet_address", "wallet_classification", "intelligence_score",
      "discovery_score", "discovery_tier", "win_rate", "average_roi",
      "conviction_score", "total_tokens_traded",
