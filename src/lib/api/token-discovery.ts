...
function _consumeHC(cuAmount: number, label: string): boolean {
  ...
  // Set to 0 (or omit) to disable the hourly cap entirely.
...
function buildSupabase() {
  ...
  // P2-D: no anon-key fallback. Discovery writes to wallet_collection_jobs,
  // which is RLS-protected — an anon-key client would silently fail every
  // write instead of raising a clear startup error.
