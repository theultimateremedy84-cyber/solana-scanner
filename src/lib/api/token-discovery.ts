...
// FIX (P0 #2, P1 #8): import from canonical singletons.
...
// FIX (P2 #11): concurrency cap for processNewToken() to prevent connection-pool saturation.
const MAX_CONCURRENT_PROCESS = 20;
let _activeProcessCount = 0;
async function _acquireProcessSlot(): Promise<boolean> {
  if (_activeProcessCount >= MAX_CONCURRENT_PROCESS) return false;
  _activeProcessCount++;
  return true;
}
function _releaseProcessSlot(): void {
...
export class TokenDiscovery {
  ...
  private async handleMessage(raw: string): Promise<void> {
    ...
    // FIX (P2 #11): gate concurrency to prevent DB connection-pool saturation.
    if (!await _acquireProcessSlot()) {
      console.warn(`${LOG} processNewToken shed — ${_activeProcessCount}/${MAX_CONCURRENT_PROCESS} active. Sig: ${signature.slice(0, 8)}…`);
      return;
    }
    void this.processNewToken(signature)
      .catch((err) => { console.error(`${LOG} processNewToken threw for sig ${signature}:`, err); })
      .finally(() => { _releaseProcessSlot(); });
  }
