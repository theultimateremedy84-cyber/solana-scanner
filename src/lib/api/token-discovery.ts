...
async function fetchBondingCurveData(
  ...
  // Retry up to 3 attempts with 8-second gaps — account may not be confirmed yet.
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!b64) {
        ...
        console.warn(
          `[TokenDiscovery] fetchBondingCurveData: account NOT FOUND at ${bondingCurveAddress.slice(0,8)}… after 3 attempts`,
        );
        return null;
      }
