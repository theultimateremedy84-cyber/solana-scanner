...
export class TokenDiscovery {
  ...
  private async scoreDiscoveredToken(mintAddress: string): Promise<{
    ...
    try {
      ...
      const honey_pot_status = isHoneypot ? "HONEYPOT" : "SAFE";
  ...
  private async recordDiscoveredToken(
    ...
    // FIX #1: compute a real risk score instead of hardcoding 0 / "LOW".
    // scoreDiscoveredToken() never throws — it returns a fallback on any error.
    const { risk_score, risk_level, honey_pot_status } = await this.scoreDiscoveredToken(mintAddress);
    ...
    if (error) {
      // Ignore duplicate-key violations — the partial unique index
      // scan_history_discovery_token_unique makes this idempotent.
      if (error.code === "23505" || error.message.includes("duplicate key")) return;
      // Log but don't throw — discovery pipeline must not be affected.
      console.warn(
      ...
      console.log(
        ...
        `token=${mintAddress.slice(0, 8)}… deployer=${deployer.slice(0, 8)}… ` +
