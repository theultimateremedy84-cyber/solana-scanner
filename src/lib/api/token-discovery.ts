...
export class TokenDiscovery {
  ...
  private async scoreDiscoveredToken(mintAddress: string): Promise<{
    ...
    try {
      ...
      // FIX (P0 #4): "CRITICAL" is not a valid scan_history CHECK constraint value.
      // The allowed values are LOW | MEDIUM | HIGH | EXTREME. Map accordingly.
      ...
      // FIX (P0 #4): "HONEYPOT" is not a valid honey_pot_status enum value.
      // Allowed: SAFE | SUSPICIOUS | HIGH RISK | CONFIRMED HONEYPOT.
