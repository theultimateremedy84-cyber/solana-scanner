...
export class TokenDiscovery {
  ...
  private connect(): void {
    ...
    // for high-volume programs (e.g. Pump.fun, ~1000+ tx/min) on the free tier.
    // atlas-mainnet delivers the same logsSubscribe protocol at full throughput.
    const wssUrl = `wss://atlas-mainnet.helius-rpc.com/?api-key=${apiKey}`;
  ...
  private async processNewToken(signature: string): Promise<void> {
    ...
    // ── Step 0: Queue depth pre-check ────────────────────────────────────────
    // Check capacity BEFORE spending 10 CUs on getTransaction. If the queue is
    // already full we know we will shed this token — no point fetching the tx.
