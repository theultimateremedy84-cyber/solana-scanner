...
// ---------------------------------------------------------------------------
// ROI dust / distortion guard — shared by computePnL() below and by
// wallet-enricher.ts's classifyWallets() (which recomputes roiMultiple
// independently from raw invested/received for CLOSED positions). Exported
// so both call sites apply the exact same thresholds; a divergence here is
// what let 16,000–20,000x multiples slip into the smart_money classification
// gate in the first place (audited 2026-07-12).
// ---------------------------------------------------------------------------
export const DUST_INVESTMENT_THRESHOLD_SOL   = 0.001; // pure dust / gas noise
// PATCH (monetization-audit issue #2 — 2026-07-14):
//   Extended from 0.02 → 0.10 SOL.  The 0.02 floor was too low: BwWK17cb-class
//   wallets with investments of 0.027–0.098 SOL were producing 863–2,532× returns
//   (above the EXTREME_ROI_MULTIPLE guard) but slipping through because they were
//   above the 0.02 floor.  At 0.10 SOL the guard covers the full suspicious band.
//   Corresponding SQL backfill: migration 20260714000005_extend_roi_distortion_guard.sql
export const SMALL_INVESTMENT_THRESHOLD_SOL  = 0.10;  // extended: was 0.02 — covers 0.02–0.10 SOL outliers
export const EXTREME_ROI_MULTIPLE            = 200;   // lowered 2026-07-18: was 500; cleanup used 200x threshold so 200-499x slipped through

export function guardRoiMultiple(
  roiMultiple: number | null,
  initialInvestment: number,
): number | null {
  if (roiMultiple == null) return null;
  if (initialInvestment > 0 && initialInvestment < DUST_INVESTMENT_THRESHOLD_SOL) {
    return null;
  }
  if (
    initialInvestment > 0 &&
    initialInvestment < SMALL_INVESTMENT_THRESHOLD_SOL &&
    roiMultiple > EXTREME_ROI_MULTIPLE
  ) {
    return null;
  }
  return roiMultiple;
...
function _consumeHC(cuAmount: number, label: string): boolean {
  ...
  // ── Daily bucket ───────────────────────────────────────────────────────────
  // FIX (P0 #5): use calendar UTC day instead of rolling 24h window.
  ...
  // every 60s. By pushing here, TxReconstructor Helius calls appear in the budget
  // dashboard alongside discovery calls — fixing the audit finding that cu_used
  // only reflected WS notifications.
  if (Array.isArray(g.__cuLogBatch__)) {
    ...
    // Trigger immediate flush if batch is getting large (matches token-discovery.ts logic)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if ((g.__cuLogBatch__ as unknown[]).length >= 50 && typeof (g as any)._flushCuLog === "function") {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (g as any)._flushCuLog();
    }
  }
...
export interface ReconstructedPosition {
  ...
  /**
   * Number of pump.fun swap transactions returned by Helius for this wallet.
   * Stored in wallet_raw_tx_metrics.total_signatures_scanned.
   * 0 for tombstone rows (no evidence).
   */
...
async function fetchWalletTokenTxs(
  ...
  const b = g.__heliusBudget__ as { budget: number; used: number; calendarDay?: string } | undefined;
...
function classifyPositionStatus(
  ...
  // Truly no evidence — no tokens bought, no tokens sold, no SOL invested.
  if (tokensBought === 0 && tokensSold === 0 && investedSol === 0) return "UNKNOWN";
  // Sell-only: the wallet sold tokens and received real SOL proceeds, but the
  // matching buy is absent from Helius history. Two causes:
  //   (a) Pre-history buy — the wallet bought before the Helius lookback window.
  //   (b) Raydium buy — the wallet bought on Raydium after pump.fun graduation;
  //       fetchWalletTokenTxs uses source=PUMP_FUN which doesn't return Raydium txs.
  // The sell is confirmed real evidence. Classify as CLOSED with zero cost basis.
  // ROI stays null (investedSol=0 guard in computePnL) so win_rate is not inflated.
  if (tokensBought === 0 && tokensSold > 0) return "CLOSED";
...
function computePnL(opts: {
  ...
  // ── Dust / distortion guard (Fix 2, extended 2026-07-12) ─────────────────
  ...
  //
  // EXTENDED 2026-07-12: a data-quality audit found the 0.001 SOL bar was too
  // low — positions around 0.006 SOL (well above pure dust) still produced
  // 16,000–20,000x multiples off early bonding-curve buys, and those raw
  // multiples fed straight into wallet-enricher.ts's smart_money gate
  // (uncapped, unlike the 10x-capped intelligence score). Any small
  // investment paired with an implausible multiple is guarded the same way.
  roiMultiple = guardRoiMultiple(roiMultiple, initialInvestment);
...
export async function reconstructWalletPosition(opts: {
  ...
  if (trades.length === 0) {
    ...
    // Still record how many sigs were scanned even though none matched this token.
    emptyResult.signaturesScanned = rawTxs.length;
    return emptyResult;
  }
  ...
  const rawBalance = Math.max(0, totalTokensBought - totalTokensSold);
  // Dust guard: if the remaining balance is < 0.1% of the total bought, it is a
  // floating-point artefact from imprecise token-amount arithmetic and must be
  // treated as zero. This is consistent with the proportional threshold used in
  // classifyWallets() (wallet-enricher.ts line ~989) so the stored balance and
  // the scorer always agree on whether a position is fully closed.
  const currentTokenBalance =
    totalTokensBought > 0 && rawBalance < totalTokensBought * 0.001 ? 0 : rawBalance;
  ...
  return {
    ...
    signaturesScanned: rawTxs.length,
  };
}
