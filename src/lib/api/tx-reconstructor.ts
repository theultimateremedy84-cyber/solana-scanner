...
/**
 * Pre-flight check for schedulers — returns true when either the hourly or
 * daily Helius credit budget is already exhausted. Lets the caller skip an
 * entire tick rather than attempting individual RPC calls that will
 * immediately short-circuit inside _consumeHC.
 *
 * Reads from globalThis, which _consumeHC initialises on first use. If the
 * buckets don't exist yet, budget has obviously not been exhausted.
 */
export function isHeliusBudgetExhausted(): boolean {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const g = globalThis as any;
  const now = Date.now();

  const b = g.__heliusBudget__ as { budget: number; used: number; day: number } | undefined;
  if (b && b.budget > 0 && now - b.day < 86_400_000 && b.used >= b.budget) return true;

  const h = g.__heliusHourly__ as { budget: number; used: number; window: number } | undefined;
  if (h && h.budget > 0 && now - h.window < 3_600_000 && h.used >= h.budget) return true;
