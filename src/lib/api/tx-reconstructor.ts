 ...
 *
 * Credit accounting: we reserve `limit` CU up-front (worst-case).
 * Helius charges 1 CU per transaction returned, not per request.
 * If the endpoint returns 12 txs, only 12 CU are actually billed —
 * the remaining reservation is a conservative over-estimate that
 * keeps the budget guard accurate without under-counting.
 *
...
async function fetchWalletTokenTxs(
  ...
  // Reserve CU budget up-front (worst-case = limit txs returned).
  if (!_consumeHC(limit, `TxReconstructor/fetchWalletTokenTxs(${walletAddress.slice(0, 8)})`)) {
    return [];
  }
  ...
  try {
    ...
    console.log(
      ...
      `(${limit} CU reserved, actual charged ≈ ${txs.length})`,
    );
    return txs;
  } catch (err) {
