...
//   Confirmed Scammer  — 3+ CRITICAL-risk tokens, or 1+ CRITICAL + very low graduation
//   Serial Offender    — 1+ CRITICAL, or 2+ HIGH-risk tokens
//   Suspicious         — 1+ HIGH-risk token
//   Clean              — no HIGH/CRITICAL launches
//
// CRITICAL risk_level is used as the "extreme" proxy because scan-core sets
// risk_level="CRITICAL" only for tokens scoring ≥ 90 (CPI, state-hijack,
// atomic-exploit forced flags), matching developer-intel's "extreme_risk" tier.
...
async function snapshotDevelopers(): Promise<number> {
  ...
  for (const row of data ?? []) {
    ...
    if (row.risk_level === "CRITICAL") devMap[dev].critical_risk++;
  }
