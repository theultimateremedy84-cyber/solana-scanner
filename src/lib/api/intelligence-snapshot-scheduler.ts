...
// ── Developer classification helper ──────────────────────────────────────────
//
// Mirrors the per-request logic in developer-intel.functions.ts so that the
// batch snapshot produces the same four-tier classification:
//   Confirmed Scammer  — 3+ EXTREME-risk tokens, or 1+ EXTREME + very low graduation
//   Serial Offender    — 1+ EXTREME, or 2+ HIGH-risk tokens
//   Suspicious         — 1+ HIGH-risk token
//   Clean              — no HIGH/EXTREME launches
//
// FIX (2026-07-20): Originally checked risk_level="CRITICAL" as the extreme proxy.
// After the P1 fix (2026-07-13), discovery-rescore-scheduler now writes "EXTREME"
// (not "CRITICAL") for tokens scoring ≥ 80. Both values are counted as critical_risk
// so that pre-fix rows (CRITICAL) and post-fix rows (EXTREME) classify correctly.
function classifyDeveloper(stats: {
  total: number; graduated: number; high_risk: number; critical_risk: number;
}): string {
  if (stats.critical_risk >= 3) return "Confirmed Scammer";
  if (stats.critical_risk >= 1 || stats.high_risk >= 2) return "Serial Offender";
  if (stats.high_risk >= 1) return "Suspicious";
  return "Clean";
}
...
async function snapshotDevelopers(): Promise<number> {
  ...
  // Aggregate from scan_history — select risk_level to distinguish HIGH vs CRITICAL
  const { data, error } = await supabaseAdmin
    .from("scan_history")
  ...
  // FIX (Issue 1 — developer_classification always null):
  // Track CRITICAL separately from HIGH so classifyDeveloper() can apply the
  // same four-tier logic used in developer-intel.functions.ts per-request paths.
  ...
  for (const row of data ?? []) {
    ...
    if (!devMap[dev]) devMap[dev] = { total: 0, graduated: 0, high_risk: 0, critical_risk: 0 };
    devMap[dev].total++;
    if (row.graduated_at) devMap[dev].graduated++;
    if (row.risk_level === "HIGH")                                       devMap[dev].high_risk++;
    // Count both "CRITICAL" (pre-fix rows) and "EXTREME" (post-fix rows) as critical_risk
    if (row.risk_level === "CRITICAL" || row.risk_level === "EXTREME") devMap[dev].critical_risk++;
  }
  ...
  const rows = Object.entries(devMap).map(([dev, stats]) => ({
    ...
    high_risk_count:        stats.high_risk + stats.critical_risk, // total HIGH+CRITICAL
    graduation_rate:        stats.total > 0 ? stats.graduated / stats.total : 0,
    // FIX: compute and write developer_classification instead of leaving it null
    developer_classification: classifyDeveloper(stats),
    snapshotted_at:         new Date().toISOString(),
  }));
