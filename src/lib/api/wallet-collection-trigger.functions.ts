...
// ---------------------------------------------------------------------------
// Supabase — canonical service-role singleton only. No anon-key fallback.
// `supabaseAdmin` throws at construction time if SUPABASE_URL /
// SUPABASE_SERVICE_ROLE_KEY are missing, so background collection jobs fail
// immediately and loudly rather than silently degrading to a weaker client.
// ---------------------------------------------------------------------------
...
function getSupabase() {
  try {
    ...
    console.error(
      `${LOG} ✗ Supabase admin client unavailable — ` +
...
export const enqueueWalletCollection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    // BACKLOG_MODE guard — this server function is fired automatically after
    // every live scan on the homepage (see routes/index.tsx), which would
    // otherwise keep enqueueing NEW wallet_collection_jobs (and immediately
    // firing background Helius collection for them) even while BACKLOG_MODE=1
    // is meant to stop all new-wallet work and reserve capacity for clearing
    // the existing hollow-wallet backlog.
    if (process.env.BACKLOG_MODE === "1") {
      console.log(
        `${LOG} ▶ SKIP — BACKLOG_MODE=1, not enqueueing new collection job for ${data.tokenAddress}`,
      );
      return {
        success:      false,
        jobId:        null,
        tokenAddress: data.tokenAddress,
        skipped:      true,
        reason:       "BACKLOG_MODE active — new wallet collection is paused",
      };
    }
    ...
    if (sb && jobId) {
      ...
      if (stampErr) {
        console.error(
          `${LOG} Step 2 stamp failed for job ${jobId}: ${stampErr.message} (code: ${stampErr.code})\n` +
          "  code=42501 = RLS rejection → SUPABASE_SERVICE_ROLE_KEY missing/wrong. " +
          "  collect() will still run but job stays pending in DB.",
        );
      }
    }
