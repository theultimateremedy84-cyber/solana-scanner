...
export async function handleProcessJobsPost(request: Request): Promise<Response> {
  ...
  // BACKLOG_MODE guard — this HTTP handler is the backup execution path for
  // wallet_collection_jobs (fired every minute by the Railway cron via
  // cron-trigger.mjs, independent of the in-process ProcessJobsScheduler that
  // server.ts already disables under BACKLOG_MODE). Without this check, the
  // cron backup would keep calling collect() — and burning Helius credits —
  // even while BACKLOG_MODE=1 is meant to reserve all capacity for
  // hollow-wallet enrichment.
  if (process.env.BACKLOG_MODE === "1") {
    console.log(`${LOG} BACKLOG_MODE=1 — skipping wallet_collection_jobs processing (no-op)`);
    return json({ ok: true, processed: 0, backlogMode: true, message: "BACKLOG_MODE active — wallet job processing paused" });
  }
  ...
  if (!stuckJobs || stuckJobs.length === 0) {
    ...
    for (const job of stuckJobs) {
      ...
      try {
        ...
        // FIX (#5 — error logging): write per-attempt error to the errors JSONB
        // column so failed jobs have queryable diagnostic data.
        const errEntry = {
