  ...
  if (enr.hollowPairsPending > 20) {
    issues.push({
      ...
      description:
        ...
        `Auto-fix: triggering full-history enrichment via /api/enrich-wallets.`,
      metric: "enrichment.hollowPairsPending",
      value: String(enr.hollowPairsPending),
      fixable: true,
      // FIX (2026-07-20): removed "rescore_wallets" from the chain.
      // The rescore step was causing the entire chain to be marked success=false
      // when the rescore call timed out or errored, opening the circuit breaker
      // and permanently blocking enrichment fixes even though enrichment itself
      // was working. The background RescoreScheduler (every 20 min) already
      // rescores all wallets — adding it to the fix chain is redundant and
      // harmful to circuit breaker stability.
      fixChain: ["trigger_enrichment"],
      detectedAt: now,
    });
  }

  // ── 6. Unenriched wallet coverage ────────────────────────────────────────
  // FIX (2026-07-20): Previously used enr.ghostEnrichments (wallet_raw_tx_metrics
  // has_evidence=false count = 82,008 tombstones) divided by totalPerformanceRecords
  // (130,745). This ratio is permanently ~63% — tombstones are by design (wallets
  // Helius queried and found empty). It can never be reduced by rescoring or
  // re-enriching, so this fired "critical" on every single agent cycle forever.
  //
  // Fix: use hollowPairsPending (wallets with WPH records but NO wallet_raw_tx_metrics
  // row at all — never enriched, genuinely fixable via trigger_enrichment). Currently
  // 1,462 / 130,745 = 1.1% → below threshold → no false-positive firing.
  // The enrichment_backlog issue (check §5 above) already tracks hollowPairsPending
  // directly and fires the correct fix chain when it exceeds 20 pairs.
  if (enr.totalPerformanceRecords > 0) {
    const unenrichedPct = Math.round(
      (enr.hollowPairsPending / enr.totalPerformanceRecords) * 100,
    );
    if (unenrichedPct > 30) {
      issues.push({
        id: randomUUID(),
        severity: unenrichedPct > 60 ? "critical" : "warning",
        category: "data_integrity",
        title: `${unenrichedPct}% of wallet performance records have never been enriched`,
        description:
          `${enr.hollowPairsPending.toLocaleString()} wallets have performance history records but ` +
          `no Helius enrichment attempt at all (threshold: 30% of ${enr.totalPerformanceRecords.toLocaleString()} total). ` +
          `Auto-fix: triggering enrichment, then rescoring to rebuild P&L from fetched data.`,
        metric: "enrichment.hollowPairsPending",
        value: `${unenrichedPct}%`,
        fixable: true,
