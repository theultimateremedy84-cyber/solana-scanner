...
export async function refreshOpenPositionPrices(opts: {
  ...
  // NOTE: PostgREST aggregate syntax (wallet_address.count()) is disabled by
  // default on Supabase ("db-aggregates-enabled" is off).  Fetching plain rows
  // and counting/ranking in JavaScript gives identical results with no config
  // change required.  The SQL equivalent of the old query was:
  //   SELECT token_address, COUNT(wallet_address) AS wallet_count
  //   FROM wallet_performance_history
  //   WHERE position_status IN ('OPEN','PARTIALLY_CLOSED')
  //   GROUP BY token_address ORDER BY wallet_count DESC LIMIT <maxTokens>;
  const { data: rows, error: tokenErr } = await sb
    .from("wallet_performance_history")
    .select("token_address")
    .in("position_status", ["OPEN", "PARTIALLY_CLOSED"])
    .limit(2000) as {           // 2 000 >> expected row count; avoids default 1 000-row cap
      data: TokenRow[] | null;
  ...
  // Count how many wallets hold each token, then sort descending and cap.
  // This replicates the GROUP BY … COUNT … ORDER BY … LIMIT of the old query.
  const tokenCount = new Map<string, number>();
  for (const row of rows) {
    tokenCount.set(row.token_address, (tokenCount.get(row.token_address) ?? 0) + 1);
  }
  const uniqueTokens = Array.from(tokenCount.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, maxTokens)
    .map(([addr]) => addr);
