// =============================================================================
// Wallet Price Refresh Worker  (v4 — open-position cap fix + null-price preservation)
//
// CHANGES FROM v2:
...
async function buildTokenList(
  ...
  // FIX (Fix 5A): the old candidates.slice(0, maxTokens) silently dropped
  ...
  // Step 2: snapshot-only tokens — fill remaining slots up to maxTokens
  const snapshotSlots = Math.max(0, maxTokens - openCandidates.length);
  const snapshotCandidates: TokenCandidate[] = [];
  for (const ta of allTokens) {
    if (!openTokenCount.has(ta)) {
      snapshotCandidates.push({ tokenAddress: ta, walletCount: 0, needsPnlUpdate: false });
      if (snapshotCandidates.length >= snapshotSlots) break;
    }
  }
