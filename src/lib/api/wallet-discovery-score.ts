...
//   elite       — score ≥ 0.65 AND confidence ≥ 0.50  (requires n≥2 positions at CONF_K=2)
//   strong      — score ≥ 0.45 AND confidence ≥ 0.25  (n=1 qualifies: confidence=0.39)
...
function aggregateWalletData(
  ...
  // Distinct tokens this wallet participated in
  const distinctTokens = new Set(positions.map((p) => p.token_address)).size;
  const n = positions.length;

  // Milestone hits — reached $1 M+ MC
  const milestoneCount = positions.filter(
    (p) => p.reached_1m_mc === true || p.reached_5m_mc === true || p.reached_10m_mc === true,
  ).length;

  // ROI quality — only positions with a positive roi_multiple
  const roiPositions = positions.filter(
  ...
  // Win rate — profitable closed positions
...
function deriveTier(score: number, confidence: number): string {
  ...
  // Threshold lowered 0.30→0.20 (at CONF_K=2, n=1 gives confidence=0.39 > 0.20).
  // low_sample: threshold lowered 0.30→0.20 so n=1 wallets (confidence=0.39) escape it.
  if (confidence < 0.20) return "low_sample";
  // elite: confidence threshold restored to 0.50. At CONF_K=2, this requires n≥2
  // positions (confidence=0.63). n=1 wallets can be "strong" but not "elite".
  if (score >= 0.65 && confidence >= 0.50) return "elite";
  // strong: n=1 qualifies (confidence=0.39 >= 0.25). Lower bar for repeated but
  // still meaningful performance.
  if (score >= 0.45 && confidence >= 0.25) return "strong";
