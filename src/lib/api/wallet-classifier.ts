...
export function classifyWallet(opts: {
  ...
  //   Exclude zero-cost-basis CLOSED positions (airdrop exits: wallet sold tokens
  //   it received via airdrop or transfer with no recorded buy) from the win-rate
  //   denominator.  These 21,530 positions have initialInvestment = 0 and
  //   roiMultiple = null — they cannot produce a meaningful win/loss signal.
  //   Counting them in the denominator was depressing win_rate for wallets that
  //   received airdropped tokens alongside their real trades.
  //   Corresponding DB flag: wallet_performance_history.is_airdrop_exit (migration
  //   20260714000006_airdrop_exit_flag.sql).
  const closedWithCostBasis = closed.filter((p) => (p.initialInvestment ?? 0) > 0.001);
  const profitableWithBasis = closedWithCostBasis.filter((p) => (p.realizedProfit ?? 0) > 0);
  const winRate: number | null =
    closedWithCostBasis.length >= 1 ? profitableWithBasis.length / closedWithCostBasis.length : null;

  // ── Average ROI (closed positions with cost basis only — realized P&L is concrete) ──
  // Also excludes zero-investment positions (they already have roiMultiple = null).
  const closedWithRoi = closedWithCostBasis.filter((p) => p.roiMultiple != null);
