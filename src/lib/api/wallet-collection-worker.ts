...
function _consumeHC(cuAmount: number, label: string): boolean {
  ...
  // FIX (P0 #5): use calendar UTC day instead of rolling 24h window so restart
  // at 23:59 doesn't re-allow the full daily budget immediately after midnight.
...
async function persistWallets(traders: ParsedTrader[], errors: string[]): Promise<void> {
  ...
  const rows = Array.from(walletMap.entries()).map(([addr, w]) => {
    ...
    return {
      ...
      // FIX (P0 #1): wallet_classification NEVER written by the collection worker.
      // Any value already in the DB (whale, smart_money, etc.) must be preserved.
      // total_tokens_traded: omitted — refreshed by refresh_wallet_token_counts() below.
      // FIX (P0 #3): total_buys/total_sells EXCLUDED from this upsert.
      // They are accumulated via increment_wallet_buy_sell_counts() RPC below
      // so that returning wallets accumulate counts instead of being reset to
      // just this job's count.
    };
  });
  ...
  // FIX (P0 #3): Increment total_buys/total_sells atomically via a DB function.
  // This accumulates counts across multiple scans rather than overwriting them.
