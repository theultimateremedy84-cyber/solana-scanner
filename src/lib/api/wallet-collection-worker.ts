...
// ---------------------------------------------------------------------------
// extractSellers — v3 (wSOL-close fix)
//
// ROOT CAUSE of "0 sellers" (diagnosed 2026-07-06):
//
//   Pump.fun sells pay the seller through a wSOL intermediate account:
//     Pool → wSOL temp account (created in same tx) → closeAccount → Seller wallet
//
//   In Helius nativeTransfers this appears as TWO hops:
//     { from: pool,          to: wSOL_account, amount: X }   ← pool pays wSOL
//     { from: wSOL_account,  to: seller_wallet, amount: Y }  ← wSOL closes to seller
//
//   v2 built `solReceived` only from pool → direct_recipient, so it saw
//   `solReceived = { wSOL_account: X }`.  Then it tried to match
//   `tokenTransfer.fromUserAccount (= seller_wallet)` against `solReceived`,
//   which has `wSOL_account`, not `seller_wallet`.  No match → 0 sellers.
//
// FIX (v3):
//   Step A — confirm the pool sent SOL at all (any native transfer where
//             fromUserAccount === poolAddress).  This is the "is it a sell?" gate.
//
//   Step B — for every wallet that sent the correct token in this tx,
//             sum up all SOL they received from ANY native transfer
//             (including the wSOL closeAccount hop).  This captures the
//             seller's payout even when it arrives via a wSOL intermediate.
// ---------------------------------------------------------------------------
function extractSellers(tx: HeliusEnhancedTx, mint: string, poolAddress: string): ParsedTrader[] {
  ...
  // ── Step A — Gate: did the pool send any SOL in this tx? ──────────────────
  // Any native transfer where fromUserAccount === poolAddress indicates
  // sell activity (pool paying out to seller, wSOL intermediate, or fee).
  const poolSentSol = native.some((n) => n.fromUserAccount === poolAddress);

  if (!poolSentSol) {
    console.log(
      `[SELLER_SCAN_V2] REJECT_NO_SOL_FROM_POOL sig=${tx.signature?.slice(0, 20)}… ` +
      `pool=${poolAddress} sent no SOL in this tx — not a sell`,
    );
    return traders;
  }

  // ── Step B — Build a map of every wallet → total SOL received in this tx ──
  // Sums across ALL native transfers (not just pool → wallet) so that
  // wSOL-close hops are captured: the seller ultimately receives SOL
  // from the wSOL account, not directly from the pool.
  const solReceivedByWallet = new Map<string, number>();
  for (const n of native) {
    const to = n.toUserAccount ?? "";
    if (to && to !== poolAddress && !IGNORED_PROGRAMS.has(to)) {
      solReceivedByWallet.set(to, (solReceivedByWallet.get(to) ?? 0) + n.amount / LAMPORTS_PER_SOL);
    }
  }

  // ── Step C — Identify sellers: sent the right token + received SOL ─────────
  // A seller is a non-pool, non-program wallet that:
  //   (a) sent tokens of the correct mint in this tx
  //   (b) received SOL from any source in the same tx (their payout)
  ...
  console.log(
    `[SELLER_SCAN_V2] extractSellers() sig=${tx.signature?.slice(0, 20)}… ` +
    `→ ${traders.length} seller(s)  ` +
    `[tokenTransfers=${transfers.length} nativeTransfers=${native.length}]`,
  );
