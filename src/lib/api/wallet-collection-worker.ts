// =============================================================================
// Wallet Collection Worker  (v9 — position tracking + correct P&L)
//
...
/**
 * Fetch the current price of a token from DexScreener.
 * Returns null values on failure — callers must handle gracefully.
 *
 * priceNative on Solana = price per token denominated in SOL.
 */
export async function fetchTokenPrice(tokenAddress: string): Promise<TokenPriceData> {
  const empty: TokenPriceData = { priceSol: null, priceUsd: null, marketCapUsd: null, fetchedAt: new Date().toISOString() };
  ...
  try {
    ...
    console.log(
      ...
      `priceSol=${priceSol ?? "N/A"} priceUsd=${priceUsd ?? "N/A"} mcap=${marketCapUsd ?? "N/A"}`,
    );

    return { priceSol, priceUsd, marketCapUsd, fetchedAt: new Date().toISOString() };
  } catch (err) {
