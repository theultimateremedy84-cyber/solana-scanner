...
function extractSellers(tx: HeliusEnhancedTx, mint: string, poolAddress: string): ParsedTrader[] {
  ...
  if (solReceived.size === 0) return traders; // no SOL left the pool — no sells in this tx
  ...
  for (const t of transfers) {
    if (t.mint !== mint || !t.fromUserAccount || IGNORED_PROGRAMS.has(t.fromUserAccount)) continue;
    if (!solReceived.has(t.fromUserAccount)) continue; // only wallets that got SOL from the pool
    const tokenAmount = Number(t.tokenAmount ?? 0);
    if (!isFinite(tokenAmount) || tokenAmount < MIN_TOKEN_AMOUNT) continue;
