...
// ---------------------------------------------------------------------------
// extractSellers — v4 (accountData balance-delta fix)
//
...
function extractSellers(tx: HeliusEnhancedTx, mint: string, poolAddress: string): ParsedTrader[] {
  ...
  if (!poolPaidSol) {
    console.log(
      `[SELLER_SCAN_V4] REJECT_NO_SOL_FROM_POOL sig=${tx.signature?.slice(0, 20)}… ` +
  ...
  console.log(
    `[SELLER_SCAN_V4] extractSellers() sig=${tx.signature?.slice(0, 20)}… ` +
