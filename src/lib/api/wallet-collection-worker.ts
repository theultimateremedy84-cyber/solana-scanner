...
export async function collect(job: WalletCollectionJob): Promise<CollectionResult> {
  ...
  try {
    ...
    // maxWallets reduced from 30 → 10 to conserve Helius CUs (each wallet costs
    // ~200-500 CUs for full tx history reconstruction). delayMs increased for
    // safer rate limiting. Set HELIUS_ENRICH_WALLETS env var to override.
    if (getHeliusKey() && allTraders.length > 0) {
      const walletAddresses = [...new Set(allTraders.map((t) => t.walletAddress))];
      const maxEnrich = parseInt(process.env.HELIUS_ENRICH_WALLETS ?? "10", 10) || 10;
