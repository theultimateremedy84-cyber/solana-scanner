...
// ---------------------------------------------------------------------------
// SOL/USD price — cached 10 min to avoid hammering CoinGecko per job run
// ---------------------------------------------------------------------------

let _cachedSolPriceUsd   = 150;   // conservative fallback if CoinGecko is down
...
async function getSolPriceUsd(): Promise<number> {
  ...
  } catch { /* non-fatal — use cached fallback */ }
...
function extractSellers(tx: HeliusEnhancedTx, mint: string, poolAddress: string): ParsedTrader[] {
  console.log(
    `[SELLER_SCAN_V2] extractSellers() ENTER sig=${tx.signature?.slice(0, 12)}… ` +
    `mint=${mint.slice(0, 8)}… pool=${poolAddress.slice(0, 8)}… ` +
    `tokenTransfers=${(tx.tokenTransfers ?? []).length} nativeTransfers=${(tx.nativeTransfers ?? []).length}`,
  );
  ...
  // Step A — Build SOL-received map from native transfers.
  // Wallets that received SOL FROM the bonding curve in this transaction are sellers.
  // This uses the raw account address so poolAddress (the PDA) matches correctly.
  ...
  if (solReceived.size === 0) {
    console.log(
      `[SELLER_SCAN_V2] EARLY RETURN — sig=${tx.signature?.slice(0, 12)}… ` +
      `solReceived.size=0 (no SOL left the pool in this tx — not a sell)`,
    );
    return traders; // no SOL left the pool — no sells in this tx
  }

  console.log(
    `[SELLER_SCAN_V2] Step A done — sig=${tx.signature?.slice(0, 12)}… ` +
    `${solReceived.size} wallet(s) received SOL from pool`,
  );

  // Step B — Confirm each SOL-recipient sent tokens of the right mint.
  //
  // FIX (Pump.fun ATA mismatch):
  //   The old check was: t.toUserAccount === poolAddress
  //   That check fails because Helius resolves the destination of a token transfer
  //   to the ATA *address*, not the ATA's owner (the bonding curve PDA).
  //   So every sell was silently discarded — sellers_collected stayed 0 even
  //   when pool_address was correctly set.
  //
  //   Fix: drop the destination check entirely. Instead, use the SOL receipt as
  //   the authoritative sell signal (Step A above) and just confirm the wallet
  //   sent the right token somewhere in the same transaction. The bonding curve
  //   is the only account that can send SOL in a Pump.fun sell, so a wallet
  //   that (a) received SOL from the pool and (b) sent the token in the same tx
  //   is unambiguously a seller.
  const sellerTokensSent = new Map<string, number>();
  let filteredWrongMint   = 0;
  let filteredNotInSolMap = 0;
  let filteredDustTokens  = 0;

  for (const t of transfers) {
    if (t.mint !== mint || !t.fromUserAccount || IGNORED_PROGRAMS.has(t.fromUserAccount)) { filteredWrongMint++; continue; }
    if (!solReceived.has(t.fromUserAccount)) { filteredNotInSolMap++; continue; } // only wallets that got SOL from the pool
    const tokenAmount = Number(t.tokenAmount ?? 0);
    if (!isFinite(tokenAmount) || tokenAmount < MIN_TOKEN_AMOUNT) { filteredDustTokens++; continue; }
    sellerTokensSent.set(
      t.fromUserAccount,
      (sellerTokensSent.get(t.fromUserAccount) ?? 0) + tokenAmount,
    );
  }

  console.log(
    `[SELLER_SCAN_V2] Step B filters — sig=${tx.signature?.slice(0, 12)}… ` +
    `confirmed_sellers=${sellerTokensSent.size} ` +
    `filtered_wrong_mint_or_program=${filteredWrongMint} ` +
    `filtered_not_in_sol_received_map=${filteredNotInSolMap} ` +
    `filtered_dust_token_amount=${filteredDustTokens}`,
  );

  let filteredDustSol = 0;
  for (const [walletAddress, tokenAmount] of sellerTokensSent.entries()) {
    const amountSol = solReceived.get(walletAddress) ?? 0;
    if (amountSol < MIN_SOL_THRESHOLD) { filteredDustSol++; continue; }
    traders.push({
      walletAddress,
  ...
  if (filteredDustSol > 0) {
    console.log(
      `[SELLER_SCAN_V2] Filtered ${filteredDustSol} seller(s) below MIN_SOL_THRESHOLD (${MIN_SOL_THRESHOLD} SOL) ` +
      `sig=${tx.signature?.slice(0, 12)}…`,
    );
  }

  console.log(
    `[SELLER_SCAN_V2] extractSellers() EXIT sig=${tx.signature?.slice(0, 12)}… → ${traders.length} seller(s) emitted`,
  );
...
async function persistActivity(
  ...
  if (sellerRows.length === 0) {
    console.warn(
      `[SELLER_SCAN_V2] ⚠ Step 4: 0 seller rows about to be written to wallet_token_activity — ` +
      `check extractSellers() logs above for filter reasons`,
    );
  } else {
    console.log(
      `[SELLER_SCAN_V2] Step 4: ${sellerRows.length} seller row(s) queued for DB write ` +
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    ...
    if (error) {
      ...
      if (chunkSellers > 0) {
        console.log(
          `[SELLER_SCAN_V2] ✓ DB write confirmed: ${chunkSellers} seller row(s) written in chunk ${Math.floor(i / CHUNK)}`,
        );
      }
    }
  }
