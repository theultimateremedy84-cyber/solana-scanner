...
// Phase 1 — Transaction-Driven Wallet Intelligence
...
//
// Writes holder_scan / pool_extraction entries into wallet_raw_tx_metrics
// so future analytics (and the scoring model) can read raw aggregates
// without re-scanning the blockchain.
//
// Safety: ignoreDuplicates: true — a wallet that was already upgraded to
// helius_full_history by the enricher will NOT be downgraded here.
// ---------------------------------------------------------------------------
...
async function persistRawMetricsBaseline(
  ...
  // Aggregate per wallet (mirrors PerfEntry logic in persistPerformanceHistory)
  ...
  const rows = Array.from(walletMap.entries()).map(([addr, w]) => ({
    ...
    total_signatures_scanned: null, // not available from pool extraction path
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    const { error } = await sb
      ...
      // ignoreDuplicates: true — never overwrite a helius_full_history entry
      // that the enricher has already written for this wallet×token pair.
      .upsert(rows.slice(i, i + CHUNK), { onConflict: "wallet_address,token_address", ignoreDuplicates: true });
    if (error) {
      if (error.message.includes("does not exist") || error.message.includes("relation")) {
        console.warn(`${LOG}   wallet_raw_tx_metrics not found — apply 20260627000001 migration first`);
        break; // Non-fatal: table absent, don't spam errors
      }
...
export async function collect(job: WalletCollectionJob): Promise<CollectionResult> {
  ...
  try {
    ...
    // ── Step 6b: baseline raw metrics (Phase 1) ───────────────────────────────
    // Writes holder_scan / pool_extraction entries into wallet_raw_tx_metrics.
    // ignoreDuplicates: true ensures we never overwrite a higher-quality
    // helius_full_history row that the enricher may have already written.
    ...
    // Runs asynchronously after the HTTP response is sent — does NOT block the
    // collect() return. For each collected wallet, fetches complete Helius tx
    // history, reconstructs all buys + sells, and updates classification scores.
    // Only fires when HELIUS_API_KEY is present; silent no-op otherwise.
    if (getHeliusKey() && allTraders.length > 0) {
      const walletAddresses = [...new Set(allTraders.map((t) => t.walletAddress))];
      void enrichWalletsForToken({
        tokenAddress:    job.tokenAddress,
        walletAddresses,
        priceData,
        maxWallets:      30,   // process up to 30 wallets per scan in background
        delayMs:         400,  // 400 ms between wallets — safe for Helius free tier
      }).then((r) => {
        console.log(
