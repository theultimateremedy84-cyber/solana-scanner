...
async function fetchExistingPerfSnapshots(
  ...
  try {
    const { data: rows } = await sb
      ...
      // FIX (P1 #6): removed .slice(0,500) cap — wallets 501+ no longer lose peak ROI timestamps.
...
async function persistRawMetricsBaseline(
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    const { error } = await sb
      ...
      // FIX (P1 #9): ignoreDuplicates:false allows stale baseline rows to refresh.
      .upsert(rows.slice(i, i + CHUNK), { onConflict: "wallet_address,token_address", ignoreDuplicates: false });
    if (error) {
...
export async function collect(job: WalletCollectionJob): Promise<CollectionResult> {
  ...
  try {
    ...
    if (job.poolAddress && heliusOk) {
      ...
      // FIX (P1 #7): removed hard .slice(0,75) cap — active tokens no longer lose traders.
      const allSigs = Array.from(new Set(pageSigs));
