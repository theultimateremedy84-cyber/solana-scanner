// =============================================================================
// Wallet Collection Worker  (v8 — Railway fix)
//
// Supabase key priority:
//   SUPABASE_SERVICE_ROLE_KEY  (service role — bypasses RLS, preferred)
//   SUPABASE_ANON_KEY          (anon fallback)
//   SUPABASE_PUBLISHABLE_KEY   (publishable alias)
//   VITE_SUPABASE_ANON_KEY     (VITE-prefixed anon)
//   VITE_SUPABASE_PUBLISHABLE_KEY (last resort)
//
// URL priority (RAILWAY FIX v8):
//   process.env.SUPABASE_URL is checked FIRST (runtime-safe).
//   import.meta.env.VITE_SUPABASE_URL is baked at Vite build time — if the
//   variable wasn't present during the Railway RAILPACK build step it resolves
//   to undefined, so it is placed LAST as a fallback.
// =============================================================================
...
export function getSupabase() {
  // RAILWAY FIX: process.env is checked first (always available at runtime).
  // import.meta.env.VITE_SUPABASE_URL is baked at Vite build time — if it
  // wasn't present during the Railway build it resolves to undefined, so it
  // is the LAST fallback rather than the first.
  const url =
  ...
  if (!url || !key) {
    console.error(
      ...
      `  Checked: SUPABASE_URL, VITE_SUPABASE_URL, VITE_SUPABASE_URL (baked), SUPABASE_SERVICE_ROLE_KEY, SUPABASE_ANON_KEY,\n` +
