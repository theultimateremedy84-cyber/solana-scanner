...
const INTERVAL_MS   = 30 * 60 * 1_000;  // regular rescore every 30 minutes
const BATCH_SIZE    = 200;               // tokens per regular tick
const MIN_AGE_HOURS = 24;               // regular rescore: tokens >24h old only
const DELAY_MS      = 200;              // ms between RugCheck calls (rate limit)
const RUGCHECK_TIMEOUT_MS = 8_000;

// FIX (P2 #13): emergency rescore for tokens in their first 4h (catches quick rugs)
const EMERGENCY_INTERVAL_MS   = 30 * 60 * 1_000;
const EMERGENCY_BATCH_SIZE    = 50;
const EMERGENCY_MAX_AGE_HOURS = 4;
const EMERGENCY_CU_BUDGET     = 50;
...
export function startDiscoveryRescoreScheduler(): () => void {
  ...
  // FIX (P2 #13): also fire emergency rescore on first warmup
  const warmup2 = setTimeout(() => {
    void tickEmergency().catch((err) =>
      console.error(`${LOG} [Emergency] Initial tick failed:`, err instanceof Error ? err.message : String(err)),
    );
  }, 95_000);

  // FIX (P2 #13): emergency rescore interval for tokens <4h old
  const emergencyHandle = setInterval(() => {
    void tickEmergency().catch((err) =>
      console.error(`${LOG} [Emergency] Tick failed:`, err instanceof Error ? err.message : String(err)),
    );
  }, EMERGENCY_INTERVAL_MS);

  return () => {
    clearTimeout(warmup);
    clearTimeout(warmup2);
    if (_intervalHandle !== null) {
      clearInterval(_intervalHandle);
      _intervalHandle = null;
    }
    clearInterval(emergencyHandle);
    console.log(`${LOG} Stopped.`);
  };
}

// =============================================================================
// FIX (P2 #13): Emergency rescore — runs every 30 min for tokens <4h old.
// Catches rug tokens before the 24h regular rescore cycle would find them.
// =============================================================================
async function tickEmergency(): Promise<void> {
  const cutoff = new Date(Date.now() - EMERGENCY_MAX_AGE_HOURS * 3_600_000).toISOString();

  const { data: mints, error } = await supabaseAdmin
    .from("scan_history")
    .select("token_address, risk_score, inserted_at")
    .gte("inserted_at", cutoff)
    .eq("needs_rescore", true)
    .order("inserted_at", { ascending: false })
    .limit(EMERGENCY_BATCH_SIZE);

  if (error) { console.warn(`${LOG} [Emergency] query failed: ${error.message}`); return; }
  if (!mints || mints.length === 0) {
    console.log(`${LOG} [Emergency] No new tokens needing rescore — skipping.`);
    return;
  }

  console.log(`${LOG} [Emergency] Rescoring ${mints.length} tokens launched in last ${EMERGENCY_MAX_AGE_HOURS}h`);
  let improved = 0, unchanged = 0, failed = 0, cuUsed = 0;

  for (const { token_address: mint, risk_score: prevScore } of mints) {
    if (cuUsed >= EMERGENCY_CU_BUDGET) {
      console.log(`${LOG} [Emergency] CU budget reached (${EMERGENCY_CU_BUDGET}) — stopping tick`);
      break;
    }
    cuUsed++;
    const result = await scoreWithRugCheck(mint as string);
    if (!result) { failed++; await new Promise((r) => setTimeout(r, DELAY_MS)); continue; }
    ...
    const { error: upErr } = await supabaseAdmin
      ...
      .update({
        ...
        honey_pot_status: result.honey_pot_status,
        last_rescored_at: new Date().toISOString(),
      })
      .eq("token_address", mint);

    if (upErr) { failed++; }
    else if (result.risk_score > (prevScore as number ?? 0)) {
      improved++;
      console.log(`${LOG} [Emergency] ⚠️  ${(mint as string).slice(0,8)}… upgraded to ${result.risk_level} (${result.risk_score})`);
    } else { unchanged++; }

    await new Promise((r) => setTimeout(r, DELAY_MS));
  }
  console.log(`${LOG} [Emergency] Done — improved:${improved} unchanged:${unchanged} failed:${failed} cu:${cuUsed}`);
}
