...
export async function handleDiscoveryStatusGet(): Promise<Response> {
  ...
  // P3-D: Live enrichment coverage for the leaderboard progress bar
  const enrichmentCoverage = await getEnrichmentCoverage().catch(() => ({
    totalWallets: 0, enrichedWallets: 0, hollowWallets: 0, coveragePct: 0,
  }));
  ...
  // M-2 FIX: distinguish intentional pause from a crash so the diagnosis message
  // is actionable ("set this env var") rather than misleading ("check your API key").
  if (process.env.ENABLE_TOKEN_DISCOVERY !== "true") {
    diagnosis.push(
      "PAUSED: TokenDiscovery is disabled because ENABLE_TOKEN_DISCOVERY is not set to 'true'. " +
      "No new tokens have been scanned since the pause was enabled. " +
      "Set ENABLE_TOKEN_DISCOVERY=true in Railway → Variables to resume. " +
      "Check enrichment.hollowWallets below before re-enabling — if the backlog is still large, " +
      "keep discovery paused until it drains below ~1,000 hollow pairs.",
    );
    ...
    JSON.stringify(
      {
        ...
        // P3-D: Enrichment coverage — how many wallets have Helius full history
        enrichment: {
          ...enrichmentCoverage,
          note: enrichmentCoverage.hollowWallets > 0
            ? `${enrichmentCoverage.hollowWallets} wallets still being enriched by the 30-minute background scheduler`
            : "All wallets have Helius full-history enrichment",
        },
        env,
        diagnosis,
      },
