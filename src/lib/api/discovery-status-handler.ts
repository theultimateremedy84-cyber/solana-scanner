...
export function handleDiscoveryStatusGet(): Response {
  ...
  if (p.messagesReceived > 0 && p.createEventsFound === 0) {
    ...
    diagnosis.push(
      `PIPELINE BLOCKED at step 4: ${p.mintsExtracted} mints detected but 0 found ` +
      `on DexScreener after 25s — tokens are not reaching DEX or DexScreener API changed`,
    );
  } else if (p.dexScreenerHit > 0 && p.liquidityPassed === 0) {
    diagnosis.push(
      `PIPELINE BLOCKED at step 5: ${p.dexScreenerHit} tokens found on DexScreener ` +
      `but all below $500 liquidity — consider lowering MIN_LIQUIDITY_USD`,
    );
  }
    ...
    JSON.stringify(
      {
        ...
        pipeline: {
          ...
          "4_dexScreenerHit":    p.dexScreenerHit,
          "5_liquidityPassed":   p.liquidityPassed,
          "6_tokensEnqueued":    p.tokensEnqueued,
          filters: {
            minLiquidityUsd:    500,
            dexScreenerDelayMs: 25000,
          },
        },
        env,
        diagnosis,
