...
async function markImplicitRugPositions(
  ...
  try {
    ...
    const { data: openPositions, error: posErr } = await sb
    ...
    if (posErr || !openPositions?.length) {
    ...
    const toClose:        ClosedCandidate[] = [];  // MC-based: price dropped < threshold
    const toCloseNoPrice: ClosedCandidate[] = [];  // No price ever recorded
    ...
    for (const pos of (openPositions as Array<{wallet_address: string; token_address: string; position_status: string; initial_investment: number; total_sol_received: number; last_updated: string}>)) {
      ...
      if (!latest) {
        // No price data ever recorded for this token — the price refresh
        // scheduler never found liquidity on DexScreener. If the position
        // has been OPEN for > 48h without any price, it is a total loss.
      ...
      // Snapshot must be stale (≥ 48h) to avoid flagging recovering tokens
      if (now - latest.snapshotAt.getTime() < THREE_DAYS_MS) continue;

      const entryMc = entryMcByToken.get(pos.token_address) ?? 0;
      if (entryMc <= 0) continue;

      if (latest.marketCap / entryMc < RUG_THRESHOLD) {
        toClose.push({ wallet_address: pos.wallet_address, token_address: pos.token_address, roi_multiple: roi });
      }
    }

    const allToClose = [...toClose, ...toCloseNoPrice];
    ...
    console.log(
      `${LOG_RUG} Marking ${toClose.length} MC-dead + ${toCloseNoPrice.length} no-price-data positions as CLOSED`,
    );
