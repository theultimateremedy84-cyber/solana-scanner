...
async function markImplicitRugPositions(
  ...
  try {
    ...
    // Paginate through all price snapshots — the table is already >83K rows so
    // the previous hard limit(50000) was silently missing ~33K rows, causing
    // tokens whose latest snapshot fell beyond page 1 to always appear alive.
    // Ordered DESC so the first occurrence per token is always the freshest.
    const latestByToken = new Map<string, { marketCap: number; snapshotAt: Date }>();
    {
      const PRICE_PAGE = 5_000;
      let pricePage = 0;
      let priceFetchOk = false;
      while (true) {
        const { data: chunk, error: priceErr } = await sb
          .from("token_price_history")
          .select("token_address, market_cap_usd, snapshotted_at")
          .not("market_cap_usd", "is", null)
          .gt("market_cap_usd", 0)
          .order("snapshotted_at", { ascending: false })
          .range(pricePage * PRICE_PAGE, (pricePage + 1) * PRICE_PAGE - 1);

        if (priceErr) {
          console.warn(`${LOG_RUG} Price fetch page ${pricePage} error: ${priceErr.message}`);
          break;
        }
        ...
        for (const row of chunk) {
          // First occurrence in DESC order = most recent snapshot for this token
        ...
        if (chunk.length < PRICE_PAGE) break; // last page reached
        pricePage++;
      }

      if (!priceFetchOk || latestByToken.size === 0) {
        console.warn(`${LOG_RUG} Could not fetch any price history rows`);
        return 0;
      }
      console.log(`${LOG_RUG} Loaded latest prices for ${latestByToken.size} tokens (${pricePage + 1} pages)`);
    }
