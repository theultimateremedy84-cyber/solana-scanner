...
async function markImplicitRugPositions(
  ...
  const RUG_THRESHOLD  = 0.03;  // < 3% of entry MC = implicit rug
  ...
  try {
    ...
    const { data: openPositions, error: posErr } = await sb
      ...
      .select("wallet_address, token_address, position_status, initial_investment")
    ...
    const now = Date.now();
    const toClose: Array<{ wallet_address: string; token_address: string }> = [];

    for (const pos of (openPositions as Array<{wallet_address: string; token_address: string; position_status: string; initial_investment: number}>)) {
      const latest = latestByToken.get(pos.token_address);
      if (!latest) continue;

      // Snapshot must be stale (≥ 3 days old) to avoid flagging recovering tokens
      ...
      if (latest.marketCap / entryMc < RUG_THRESHOLD) {
        toClose.push({ wallet_address: pos.wallet_address, token_address: pos.token_address });
      }
    }
    ...
    console.log(`${LOG_RUG} Marking ${toClose.length} dead-token positions as CLOSED`);
    ...
    for (let i = 0; i < toClose.length; i += CHUNK) {
      ...
      const settledResults = await Promise.allSettled(
        chunk.map(({ wallet_address, token_address }) =>
          sb
            .from("wallet_performance_history")
            .update({
