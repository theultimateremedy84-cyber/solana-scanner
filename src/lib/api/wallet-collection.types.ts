...
export interface WalletCollectionJob {
   ...
   * Optional — when absent the worker skips trade-history collection
   * and falls back to significant-holder collection only.
   */
  ...
  attempts: number;
  /** Supabase UUID for the persisted wallet_collection_jobs row (set after insert). */
  dbJobId?: string | null;
}
