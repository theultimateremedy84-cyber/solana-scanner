...
// ---------------------------------------------------------------------------
// NOTE: the old one-shot startRescoreOnBoot() (fired once, 10s after boot,
// then never again) has been superseded by rescore-scheduler.ts, which fires
// the same 10s-after-boot rescore AND repeats it every 20 minutes for the
// life of the process. See src/lib/api/rescore-scheduler.ts and its usage in
// src/server.ts. This fixes the staleness gap where win_rate/average_roi
// froze at whatever partial enrichment state existed 10s after boot.
// ---------------------------------------------------------------------------
