...
//
// FIX (2026-07-20): Previously used a hardcoded external Railway URL without
// attaching x-cron-secret — the endpoint requires CRON_SECRET, so every call
// returned 401 and metric_after was always null. All fixes were logged as
// "non-improving", eventually opening every circuit breaker.
// Fix: use localhost (same pattern as agent-runner.ts) and attach cronHeaders().
...
async function fetchMetricAfterFix(
  ...
  try {
    const port = process.env.PORT ?? "3000";
    const res = await fetch(`http://localhost:${port}/api/monitor-dashboard`, {
      headers: { ...cronHeaders(), Accept: "application/json" },
