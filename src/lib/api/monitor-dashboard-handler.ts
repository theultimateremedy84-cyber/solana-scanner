...
export async function handleMonitorDashboardGet(): Promise<Response> {
  try {
    ...
    const [
      ...
      sb.from("wallet_token_activity").select("amount_sol.sum()").eq("action_type", "buy").gte("timestamp", last24h),
      sb.from("wallet_token_activity").select("amount_sol.sum()").eq("action_type", "sell").gte("timestamp", last24h),

      // Raw TX aggregates
      sb.from("wallet_raw_tx_metrics").select("total_sol_invested.sum()"),
      sb.from("wallet_raw_tx_metrics").select("total_sol_received.sum()"),
      ...
      sb.from("wallets").select("win_rate.avg(), average_roi.avg(), realized_pnl.sum(), unrealized_pnl.sum()"),
    ...
    const buyVolSol  = Number((buyVolumeLast24hRes.data  as VolRow[] | null)?.[0]?.amount_sol  ?? 0);
    const sellVolSol = Number((sellVolumeLast24hRes.data as VolRow[] | null)?.[0]?.amount_sol ?? 0);

    // Raw TX aggregates
    const totalSolInvested = Number((solInvestedRes.data as Array<{ total_sol_invested: number | null }> | null)?.[0]?.total_sol_invested ?? 0);
    const totalSolReceived = Number((solReceivedRes.data as Array<{ total_sol_received: number | null }> | null)?.[0]?.total_sol_received ?? 0);

    // Wallet aggregates
    type WalletAgg = { win_rate: number | null; average_roi: number | null; realized_pnl: number | null; unrealized_pnl: number | null };
    const walletAgg = (walletsAvgStatsRes.data as WalletAgg[] | null)?.[0] ?? null;
