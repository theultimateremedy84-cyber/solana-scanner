  ...
  if (rawRows?.length) {
    ...
    for (let ci = 0; ci < batchAddresses.length; ci += CLOSED_CHUNK) {
      ...
      const { data: closedRows } = await sb
        .from("wallet_performance_history")
        .select("wallet_address, token_address, initial_investment")
        .in("wallet_address", slice)
      ...
      for (const row of closedRows ?? []) {
        ...
        // Build supplement map: keep the first (and usually only) investment
        // value per wallet×token pair — don't overwrite with a later value.
