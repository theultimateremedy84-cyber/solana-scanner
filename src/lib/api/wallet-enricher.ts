...
export interface ExistingPerfRow {
...
// EXPORTED (test-only): allows the Vitest suite to exercise the upsert
// payload directly with a mocked Supabase client, without needing to drive
// the full enrichWalletsForToken() pipeline (Helius RPC, tx reconstruction,
// etc). Not intended for use outside this module in application code.
export async function upsertPerformanceRow(
  sb:           ReturnType<typeof createClient>,
  position:     Awaited<ReturnType<typeof reconstructWalletPosition>>,
  priceData:    TokenPriceData,
