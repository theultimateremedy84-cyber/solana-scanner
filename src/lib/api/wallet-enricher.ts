...
// ---------------------------------------------------------------------------
const ENRICHER_CONCURRENCY = parseInt(process.env.ENRICHER_CONCURRENCY ?? "5", 10);
    ...
    const positions = rows.map((r) => {
      ...
      if (usingRaw) {
        ...
        if (dataSource === "holder_scan" && invested === 0) {
          ...
          // BUY-EVIDENCE-INCOMPLETE (patch 2026-07-10 — hollow-pairs/win-rate
          // accuracy): a sell was recorded with zero matching buys. This is
          // NOT a phantom sell — extractSellers v5 already guards against
          // that (requires tokens sent directly to the pool). It happens
          // because the pool signature scan is capped at 3 pages (~3.1K sigs,
          // cut from 6 to conserve Helius CU budget): for high-volume pools
          // the wallet's original buy can fall outside that lookback window
          // by the time the job runs, while a later sell is still in range.
          // The sell is real; only the buy evidence is missing. Treat as
          // UNKNOWN (not PARTIALLY_CLOSED) so it isn't scored as a fabricated
          // win/loss with no cost basis, and doesn't masquerade as a wallet
          // with real evidenced trade history (see hasTransactionEvidence
          // below — this keeps such positions out of `evidenced`/`closed`,
          // so classification correctly falls back to "unknown" instead of
          // "retail", and win_rate/average_roi are never computed from them).
          posStatus = "UNKNOWN";
