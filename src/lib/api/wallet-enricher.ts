...
async function fetchWalletsForToken(
  ...
  // FIX (Issue 6 — Helius enrichment activity filter):
  //   Exclude wallets that already have a helius_full_history row in
  //   wallet_raw_tx_metrics. These wallets have been through the Helius
  //   pipeline and either:
  //     (a) have_evidence=false → dust/airdrop wallet, no tx history found
  //     (b) have_evidence=true  → already enriched, position_status updated
  //   Either way, re-enriching them wastes Helius CUs with no benefit.
  //   The hollow pairs path in find_hollow_pairs() handles (a); this guard
  //   handles (b) for the direct fetchWalletsForToken() call path.
  if (addrs.length > 0) {
    const { data: alreadyEnriched } = await sb
      .from("wallet_raw_tx_metrics")
      .select("wallet_address")
      .in("wallet_address", addrs.slice(0, 500))
      .eq("data_source", "helius_full_history");

    if (alreadyEnriched && alreadyEnriched.length > 0) {
      const enrichedSet = new Set<string>(alreadyEnriched.map((r) => r.wallet_address as string));
      const filtered = addrs.filter((a) => !enrichedSet.has(a));
      console.log(
        `[wallet-enricher] fetchWalletsForToken(${tokenAddress.slice(0, 8)}…): ` +
        `${addrs.length} candidates → ${filtered.length} after excluding ${enrichedSet.size} already-enriched wallets`,
      );
      return filtered;
    }
  }
...
async function updateWalletStats(
  ...
  //
  //   realized_pnl is intentionally NOT written here. This per-token enrichment
  //   path only sees a single position at a time and would overwrite the correct
  //   cross-token sum computed by classifyWallets() (which runs at the end of
  //   every enrichment batch and every rescore pass). Writing a single-token
  //   value here would corrupt the accumulated sum every time a new token is
  //   enriched. classifyWallets() is the single source of truth for realized_pnl.
...
export async function classifyWallets(
  ...
  // ── Per-wallet fallback (BUG-FIX 2026-07-16) ────────────────────────────────
  // ROOT CAUSE: the batch-level fallback condition (rawErr || !rawRows?.length)
  // only triggers when NONE of the wallets in the batch have raw metrics.
  // In a mixed batch where some wallets have wallet_raw_tx_metrics rows and
  // some do not, wallets without raw rows are silently absent from byWallet —
  // never processed, never re-scored, total_buys frozen at stale DB values.
  //
  // Live consequence: 29/30 top wallets by intelligence_score showed total_buys=0
  // despite having 700–800 SOL realized PnL. These wallets only exist in
  // wallet_performance_history (enriched before wallet_raw_tx_metrics existed).
  //
  // Fix: identify walletAddresses missing from byWallet and fetch their data
  // from wallet_performance_history individually. Tag rows with _isFallback=true
  // so per-wallet usingRaw detection in the loop below works correctly.
  {
    const rawWalletSet = new Set(byWallet.keys());
    const missingWallets = walletAddresses.filter(addr => !rawWalletSet.has(addr));
    if (missingWallets.length > 0) {
      const PER_WALLET_CHUNK = 200;
      for (let ci = 0; ci < missingWallets.length; ci += PER_WALLET_CHUNK) {
        const slice = missingWallets.slice(ci, ci + PER_WALLET_CHUNK);
        ...
        for (const row of fallbackData ?? []) {
          const key = row.wallet_address as string;
          if (!byWallet.has(key)) byWallet.set(key, []);
          // _isFallback tag allows per-wallet usingRaw detection in the loop below
          byWallet.get(key)!.push({ ...(row as Record<string, unknown>), _isFallback: true });
        }
      }
    }
  }
  ...
  if (allTokenAddrs.size > 0) {
    ...
    // PATCH (monetization-audit issue #5 — 2026-07-14): keep in sync with
    // wallet-rescoring.ts markImplicitRugPositions. 48 h staleness, 5% MC floor.
    const THREE_DAYS_MS = 2 * 24 * 60 * 60 * 1000;  // was 3 days — now 48 h
    const RUG_THRESHOLD = 0.05;  // was 0.03 — now 5% of entry MC
    ...
    try {
      ...
      // Mark dead tokens — Path A: stale snapshot (C-2 fix — dark = dead).
      // Previously this loop required MC < RUG_THRESHOLD (5% of entry), which left
      // two gaps mirroring the markImplicitRugPositions C-2 gap:
      //   (a) tokens with stale snapshot but unknown entry MC were skipped entirely
      //   (b) tokens rugged via liquidity removal (not sell pressure) kept a
      //       "reasonable" last MC and were never added to deadTokens
      //
      // C-2 rule: if the most recent snapshot in token_price_history is ≥ 48h old,
      // the token has gone dark on DexScreener — no liquidity to exit, practical
      // total loss, regardless of what the last MC reading was.
      // Scoring-time mirror of the markImplicitRugPositions C-2 change.
      for (const [tokenAddr, latest] of latestMc.entries()) {
        if (latest.age < THREE_DAYS_MS) continue;  // snapshot < 48h → token still tracked, may recover
        // Token dark for ≥ 48h — treat as dead for scoring purposes
        deadTokens.add(tokenAddr);
      }

      // Mark dead tokens — Path B: no price data at all (scoring-time mirror of
      // markImplicitRugPositions Path B). If a token has never appeared in
      // token_price_history it has never had detectable on-chain liquidity —
      // every OPEN position on it is a total loss. We skip the 48 h age gate
      // here (markImplicitRugPositions already applies it when writing CLOSED to
      // the DB) because if there is literally zero price history the token was
      // illiquid from the moment of collection.
      for (const tokenAddr of tokenList) {
        if (!latestMc.has(tokenAddr)) {
          deadTokens.add(tokenAddr);
        }
      }
    } catch {
  ...
  // WIN-RATE FIX: supplement map for raw path invested=0 positions.
  // The pump.fun SWAP filter misses Raydium buys. When a wallet's raw_tx_metrics
  // row shows tokensSold>0 but total_sol_invested=0, the buy is absent from the
  // raw path — but wallet_performance_history may have recorded initial_investment
  // from an earlier enrichment run or a holder_scan estimate. Supplementing here
  // allows closedWithCostBasis (in wallet-classifier.ts) to include these positions
  // in win_rate rather than silently excluding them as zero-cost-basis closes.
  // Key format: "wallet_address:token_address" → initial_investment (SOL).
  const wphInvestMap   = new Map<string, number>();
  const wphReceivedMap = new Map<string, number>();
  if (rawRows?.length) {
    ...
    for (let ci = 0; ci < batchAddresses.length; ci += CLOSED_CHUNK) {
      ...
      // Exclude zero-investment CLOSED positions (airdrop exits) from the
      // closed position count used by computeConfidenceTier(). A wallet with
      // 50 airdrop exits should not appear to have 50 "evidenced closes".
      // Use initial_investment > 0 as the proxy (DB flag is_airdrop_exit also
      // covers this but the column may not exist on older DB versions).
      //
      // Also select token_address + initial_investment to build wphInvestMap
      // for the win-rate supplement (see above).
      const { data: closedRows } = await sb
        .from("wallet_performance_history")
        .select("wallet_address, token_address, initial_investment, current_value")
        .in("wallet_address", slice)
      ...
      for (const row of closedRows ?? []) {
        ...
        // Build supplement maps: keep the first (and usually only) value per
        // wallet×token pair — don't overwrite with a later value.
        const posKey = `${wa}:${row.token_address as string}`;
        if (!wphInvestMap.has(posKey)) {
          wphInvestMap.set(posKey, Number(row.initial_investment));
        }
        // RAYDIUM-SELL FIX: also capture SOL received from WPH (current_value).
        // wallet_raw_tx_metrics uses source=PUMP_FUN which misses Raydium sells —
        // total_sol_received stays 0 even when the wallet exited profitably via
        // Raydium after token graduation. WPH records the actual proceeds.
        if (!wphReceivedMap.has(posKey) && Number(row.current_value ?? 0) > 0) {
          wphReceivedMap.set(posKey, Number(row.current_value));
        }
      }
    }
  }
  ...
  for (const [walletAddr, rows] of byWallet.entries()) {
    // Per-wallet usingRaw: check whether this wallet's rows came from
    // wallet_raw_tx_metrics (true) or the wallet_performance_history fallback
    // (false). Previously batch-level (rawRows?.length ? true : false), which
    // meant wallets fetched via the per-wallet fallback above were incorrectly
    // treated as raw-path wallets.
    const usingRaw = !rows.some(r => r._isFallback === true);
    ...
    const positions = rows.map((r) => {
      ...
      const rawInvested  = Number(usingRaw ? r.total_sol_invested : r.initial_investment) ?? 0;
      const rawReceived  = Number(usingRaw ? r.total_sol_received : r.current_value)      ?? 0;
      const totalBuyTxs  = Number(usingRaw ? r.total_buy_txs      : tokensBought)         ?? 0;
      const balance      = Math.max(0, tokensBought - tokensSold);

      // WIN-RATE FIX: for sell-only CLOSED positions in the raw path where
      // total_sol_invested=0, check wallet_performance_history for a recorded
      // initial_investment. The raw path uses source=PUMP_FUN&type=SWAP which
      // misses Raydium buys; wph may have captured the cost basis from an
      // earlier enrichment run or holder_scan estimate.
      // Only applies when: usingRaw=true AND raw invested=0 AND the token has
      // a wph row with initial_investment > 0.
      const posKey   = `${walletAddr}:${r.token_address as string}`;
      const invested = (usingRaw && rawInvested === 0 && wphInvestMap.has(posKey))
        ? wphInvestMap.get(posKey)!
        : rawInvested;

      // RAYDIUM-SELL FIX: wallet sold on Raydium after graduation — pump.fun
      // SWAP filter misses Raydium sells, so total_sol_received stays 0 in raw
      // metrics even though the wallet exited profitably. WPH (current_value)
      // captures the actual Raydium proceeds from earlier enrichment runs.
      // Only supplements when: usingRaw=true AND raw received=0 AND WPH has a
      // confirmed value > 0 for this wallet×token pair.
      const received = (usingRaw && rawReceived === 0 && wphReceivedMap.has(posKey))
        ? wphReceivedMap.get(posKey)!
        : rawReceived;
    ...
    const scores = classifyWallet({
      ...
      totalVolumeSoldSol,
      evidenceQuality:     usingRaw ? "raw" : "fallback",
    });

    // ── Win-rate fallback (BUG-FIX: classifyWallet returns winRate=null for
    // raw-evidence wallets, blocking smart_money/sniper promotion) ────────────
    // Mirror of the same fix in scripts/audit-rescore-v2.ts. Compute win_rate
    // directly from positions so it is never null for scored wallets.
    const _closedWithSOL = positions.filter(
      (p) => p.positionStatus === "CLOSED" && p.totalSolReceived > 0,
    );
    const _profitablePositions = _closedWithSOL.filter(
      (p) => p.totalSolReceived > p.initialInvestment,
    );
    const _computedWinRate = _closedWithSOL.length > 0
      ? _profitablePositions.length / _closedWithSOL.length
      : null;
    // Prefer classifier value (may be more precise); fall back to direct computation.
    const resolvedWinRate = scores.winRate ?? _computedWinRate;
    ...
    // ── H-1 GUARD: no confirmed buy evidence → force 'unknown'/'unrated' ──────
    //
    // ROOT CAUSE (Phase 1 fix, 2026-07-14):
    //   The fallback path (usingRaw=false) reads wallet_performance_history which
    //   can surface sell-only positions — positions where total_tokens_sold > 0
    //   but initial_investment = 0 (no confirmed SOL buy ever recorded by Helius).
    //   These pass hasTransactionEvidence=true (CLOSED !== UNKNOWN) so the
    //   classifier returns 'retail', writing a named classification to a wallet
    //   that has never made a confirmed on-chain purchase. The raw path can also
    //   produce totalBuyTxs=0 for wallets whose raw metrics rows are all sells.
    //
    //   Without this guard, every rescore tick re-introduces the same corrupt
    //   classifications that migration 20260714000002 cleaned up one-time.
    //
    // RULE:
    //   - Raw path (usingRaw=true):     must have totalBuyTxs > 0
    //   - Fallback path (usingRaw=false): must have at least one position with
    //     initial_investment > 0 (real SOL was provably at risk)
    //
    // Wallets that fail this check are written as 'unknown'/'unrated' with null
    // win_rate/average_roi/conviction_score so they cannot skew the leaderboard.
    // They will be re-classified correctly once Helius enrichment confirms a real
    // buy transaction (enrich-unenriched-scheduler picks them up within 30 min).
    const hasConfirmedBuyEvidence = usingRaw
      ? totalBuyTxs > 0
      : positions.some((p) => p.initialInvestment > 0);

    // PATCH (Phase 1 — Task 1, 2026-07-15):
    // The existing guard above catches wallets with zero investment, but
    // fallback-evidence wallets that DO have WPH investment can still reach
    // smart_money / sniper / whale / bot because the fallback path uses
    // rows.length as a transaction-count proxy, which saturates the 5-buy
    // premium gate for any wallet with 5+ WPH positions.
    //
    // Reality check from live data: 62 wallets scored 0.88–0.94 as smart_money
    // with total_buys = 0 and evidence_quality = 'fallback'. Their win rates and
    // ROI come from holder-scan-inferred positions, not Helius-verified trades.
    //
    // Rule: premium labels (smart_money, sniper, whale) require a Helius-verified
    // buy transaction count (usingRaw = true, totalBuyTxs > 0). The bot label
    // is also suppressed on the fallback path because the bot heuristic uses a
    // sell/buy ratio that is meaningless when both counts are the same
    // position-count proxy value. Fallback wallets with buy evidence are capped
    // at 'retail'; without buy evidence they fall to 'unknown' (guard above).
    const PREMIUM_CLASSIFICATIONS = new Set(["smart_money", "sniper", "whale", "bot"]);
    const rawClassification = hasConfirmedBuyEvidence ? scores.classification : "unknown" as const;
    const effectiveClassification = (
      !usingRaw && PREMIUM_CLASSIFICATIONS.has(rawClassification as string)
        ? "retail"  // demote: needs Helius tx verification before a premium label is earned
        : rawClassification
    ) as typeof rawClassification;

    const effectiveConfidenceTier  = hasConfirmedBuyEvidence ? confidenceTier : "unrated" as const;
    // scores.intelligenceScore is now number | null — null when the hard gate
    // in classifyWallet fired (no real exits). Propagate null to DB so the wallet
    // is fully excluded from the leaderboard (vs. 0 which appears in admin views).
    const effectiveScore           = hasConfirmedBuyEvidence ? (scores.intelligenceScore ?? null) : null;
    const effectiveWinRate         = hasConfirmedBuyEvidence ? resolvedWinRate        : null;
    const effectiveAvgRoi          = hasConfirmedBuyEvidence ? scores.averageRoi      : null;
    const effectiveConviction      = hasConfirmedBuyEvidence ? scores.convictionScore : null;
    // ── End H-1 GUARD ─────────────────────────────────────────────────────────

    // REALIZED-PNL FIX: aggregate realized profit across all positions for this
    // wallet and write it to wallets.realized_pnl. Previously updateWalletStats()
    // wrote this field per-token enrichment — always the LAST enriched token's
    // value, overwritten each run. classifyWallets() is the correct place because
    // it sees ALL positions cross-token and can produce an accurate sum.
    // Zero-cost-basis positions contribute 0 (correct — no real capital at risk).
    const totalRealizedPnl = positions.reduce((s, p) => s + (p.realizedProfit ?? 0), 0);

    updates.push({
      wallet_address:        walletAddr,
      wallet_classification: effectiveClassification,
      intelligence_score:    effectiveScore !== null ? effectiveScore / 100 : null,
      win_rate:              effectiveWinRate,
      average_roi:           effectiveAvgRoi,
      conviction_score:      effectiveConviction,
      realized_pnl:          totalRealizedPnl,
      confidence_tier:         effectiveConfidenceTier,
      closed_position_count:   hasConfirmedBuyEvidence ? closedPositionCountForTier : 0,
      evidence_quality:        evidenceQuality,
      score_computed_at:       new Date().toISOString(),
      // total_buys / total_sells — always written to satisfy the NOT NULL
      // constraint on new wallet inserts (upsert INSERT path hits this when a
      // wallet exists in wallet_performance_history but not yet in wallets).
      //
      // For the raw path (usingRaw=true): real per-tx counts from WRM — accurate.
      // For the fallback path (usingRaw=false): rows.length (distinct token
      //   position count, computed at line ~1348 above) — a conservative proxy
      //   that avoids the old "token quantity as tx count" corruption problem.
      //   This is safe to write: it is NOT the raw token quantity (millions),
      //   and for INSERT it is better than triggering a NOT NULL violation that
      //   aborts the entire upsert batch.
      //
      // BUG-FIX (2026-07-17): the previous conditional spread `...(usingRaw ? {...} : {})`
      // omitted total_buys/total_sells entirely for fallback wallets. On UPDATE
      // of an existing row this was harmless (DB preserves the existing value).
      // On INSERT of a new wallet row it caused:
      //   "null value in column total_buys of relation wallets violates not-null constraint"
      // crashing the upsert for every new wallet touched by the enricher.
      total_buys:  totalBuyTxs,
      total_sells: totalSellTxs,
      total_tokens_traded:   rows.length,
      updated_at:            new Date().toISOString(),
    });
  }

  // Batch upsert — classification data + realized_pnl cross-token aggregate
  let classified = 0;
  const CHUNK = 100;
