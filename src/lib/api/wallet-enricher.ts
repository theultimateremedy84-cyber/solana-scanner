...
// ---------------------------------------------------------------------------
// SOL transfer indexing — gated to whale/smart_money wallets only.
//
// WHY GATED: indexWalletSolTransfers() costs Helius credits per wallet. This
// enrichment loop touches every wallet trading a token (often mostly retail),
// but the fund-distribution-tracing use case only cares about whales/smart
// money moving money around. Gating here keeps credit usage focused on the
// wallets that actually matter, instead of indexing all 1500+ wallets.
// ---------------------------------------------------------------------------
const SOL_TRANSFER_INDEX_CLASSIFICATIONS = new Set(["whale", "smart_money"]);
...
export async function enrichWalletsForToken(opts: {
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      ...
      // Gated to whale/smart_money — see SOL_TRANSFER_INDEX_CLASSIFICATIONS.
      // Best-effort: never allowed to fail or slow down the core enrichment
      // guarantees above. Swallows its own errors.
      const classification = classificationMap.get(walletAddress);
      if (classification && SOL_TRANSFER_INDEX_CLASSIFICATIONS.has(classification)) {
        try {
          ...
          if (solResult.errors.length > 0) {
            ...
            console.log(
              `${LOG} sol-transfer-index ${walletAddress.slice(0, 8)}… ` +
              `(${classification}): stored ${solResult.transfersStored} transfers`,
            );
          }
        } catch (solErr) {
          console.warn(
...
/**
 * Fetch each wallet's existing classification (from a prior scoring pass)
 * so we can gate the SOL-transfer indexer to whale/smart_money wallets only.
 * Non-fatal on failure — gating simply defaults to "don't index".
 */
async function fetchWalletClassifications(
  ...
  try {
    const { data } = await sb
      ...
      .in("wallet_address", walletAddresses.slice(0, 500));
    for (const row of data ?? []) {
      const c = row.wallet_classification as string | null;
      if (c) map.set(row.wallet_address as string, c);
    }
