...
function _consumeHC(cuAmount: number, label: string): boolean {
  ...
  if (b.budget <= 0) return true; // unlimited mode
  if (b.used + cuAmount > b.budget) {
    if (!b.warned) {
      b.warned = true;
      console.warn(
...
export class PostLaunchWatcher {
  ...
  private async handleMessage(raw: string): Promise<void> {
    ...
    // All getTransaction calls are individually gated by the shared daily
    // credit budget inside each extract helper and before the CPI fetch below.
    ...
    if (hasDeepCpi && _consumeHC(10, "PLW/deepCpiCheck")) {
      try {
