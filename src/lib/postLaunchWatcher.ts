...
export class PostLaunchWatcher {
  ...
  private subscribe(): void {
     ...
     * The SystemProgram is invoked by virtually every tx, so we keep the
     * filter on the Token Program and instead scan inner instructions
     * for SystemProgram Allocate/AllocateWithSeed (the Reallocate path).
     */
  ...
  private async handleMessage(raw: string): Promise<void> {
    ...
    if (!mentionsSetAuthority && !mentionsResize) return;
