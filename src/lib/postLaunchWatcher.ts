...
// P2-D follow-up: use the canonical service-role singleton instead of a
// local createClient call with an anon-key fallback chain.
...
function buildSupabaseServerClient() {
  try {
    // Force the lazy Proxy to initialise now — supabaseAdmin throws on first
    // property access if SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY are missing,
    // which we catch here and convert to null (soft-fail: DB writes skipped,
    // Watcher continues monitoring without persisting alerts).
