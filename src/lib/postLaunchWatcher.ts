...
const _P25519 = (2n ** 255n) - 19n;

/** d constant for ed25519: −121665/121666 mod p. */
const _D25519 = (() => {
  function _inv(a: bigint, m: bigint): bigint {
  ...
  return ((-121665n % _P25519 + _P25519) % _P25519 * _inv(121666n, _P25519)) % _P25519;
})();
...
export class PostLaunchWatcher {
  ...
  // ── RPC rate limiter (credit budget guard) ──────────────────────────────────
  /**
   * Hard cap on getTransaction calls per minute across the entire watcher.
   * Helius free tier: ~1M credits/month ≈ 694/min total budget for the whole app.
   * Reserving 200/min for the watcher leaves headroom for TokenDiscovery and scans.
   * Override with HELIUS_PLW_RPC_PER_MIN env var.
   */
  private readonly rpcPerMinCap: number = (() => {
    const env = parseInt(process.env.HELIUS_PLW_RPC_PER_MIN ?? "", 10);
    return Number.isFinite(env) && env > 0 ? env : 200;
  })();
  private rpcCallsThisWindow = 0;
  private rpcWindowStart     = Date.now();

  /**
   * Returns true and increments the counter if a getTransaction call is allowed
   * within the current per-minute window. Returns false (and logs a warning once
   * per window) when the cap is reached.
   */
  private allowRpcCall(): boolean {
    const now = Date.now();
    if (now - this.rpcWindowStart >= 60_000) {
      this.rpcWindowStart     = now;
      this.rpcCallsThisWindow = 0;
    }
    if (this.rpcCallsThisWindow >= this.rpcPerMinCap) {
      if (this.rpcCallsThisWindow === this.rpcPerMinCap) {
        console.warn(
          `[PostLaunchWatcher] RPC rate cap reached (${this.rpcPerMinCap}/min) — ` +
          "skipping getTransaction calls until next minute window. " +
          "Raise HELIUS_PLW_RPC_PER_MIN or reduce MAX_TRACKED_TOKENS.",
        );
      }
      this.rpcCallsThisWindow++;
      return false;
    }
    this.rpcCallsThisWindow++;
    return true;
  ...
  private async loadTrackedMints(): Promise<void> {
    ...
    try {
      // Limit to MAX_TRACKED_TOKENS rows. Ordered by scan time descending so
      // the most recently discovered tokens are watched first when the table
      // exceeds the cap. Fall back to unordered if the column doesn't exist.
  ...
  private async handleMessage(raw: string): Promise<void> {
    ...
    this.notificationsPerMint.set(
      mintAddress,
      (this.notificationsPerMint.get(mintAddress) ?? 0) + 1,
    );
    ...
    // CPI depth >= 3 is ONLY possible when logs contain " invoke [3]" or deeper.
    ...
    const hasDeepCpi = !isMetadataSub && logs.some(
      (l: string) => l.includes(" invoke [3]") || l.includes(" invoke [4]"),
    );

    const needsRpc = mentionsSetAuthority || mentionsResize || mentionsMetadataUpdate || hasDeepCpi;
    if (!needsRpc) return;

    // Rate-limit all getTransaction calls (credit budget guard).
    if (!this.allowRpcCall()) return;
    ...
    // Only run when the log pre-filter confirms depth >= 3 and on mint
    // subscriptions only (not metadata PDA subs) to avoid duplicate fetches.
    if (hasDeepCpi) {
      try {
