 ...
 *   [ "metadata", TOKEN_METADATA_PROGRAM_ID bytes, mint bytes, bump_byte ]
 *
 * The function tries bump seeds 255 → 0 and returns the first 32-byte hash
 * that is off the ed25519 curve, base58-encoded. Returns null only when no
 * valid PDA can be found (extremely unlikely in practice) or when the mint
 * address is invalid base58.
 *
 * Result is cached at call-site; this is only called once per unique mint.
 */
function deriveMetadataPDA(mintAddress: string): string | null {
  try {
    ...
    const seed2  = Buffer.from(programIdBytes);
    const seed3  = Buffer.from(mintBytes);
    const suffix = Buffer.from("ProgramDerivedAddress", "utf8");
    ...
    for (let bump = 255; bump >= 0; bump--) {
      const hash = createHash("sha256")
        ...
        .update(Buffer.from(programIdBytes))  // program_id appended by Solana runtime
        .update(suffix)
        .digest();

      if (!_isOnEd25519Curve(new Uint8Array(hash))) {
...
export class PostLaunchWatcher {
  ...
  /**
   * Derive and subscribe to the Token Metadata PDA for a mint.
  ...
  private subscribeMetadataPDA(mint: string): void {
    ...
    if (!pdaAddress) {
      pdaAddress = deriveMetadataPDA(mint) ?? undefined;
      if (!pdaAddress) {
        console.warn(
  ...
  private async handleMessage(raw: string): Promise<void> {
    ...
    // Only run on mint subscriptions (not metadata PDA subs) to avoid duplicate
    // getTransaction fetches when both subscriptions fire on the same tx.
    if (!isMetadataSub) {
      try {
