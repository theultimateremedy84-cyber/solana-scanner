...
export class PostLaunchWatcher {
   ...
   *   2. scanned_at >= now - 3 days  — ignore stale manual scans from weeks ago.
   *   3. ORDER BY scanned_at DESC    — most recently discovered tokens fill slots first.
   */
  private async loadTrackedMints(): Promise<void> {
    ...
    try {
      ...
      const { data, error } = await this.supabase
        ...
        .like("token_address", "%pump")   // pump.fun tokens only
        .gte("scanned_at", cutoff)        // scanned within the last 3 days
