...
export class PostLaunchWatcher {
  ...
  private metadataSubscriptionId: number | null = null;
  ...
  async start(): Promise<void> {
    ...
    // Node.js < 22 has no native WebSocket global. Polyfill it once at startup.
    if (typeof globalThis.WebSocket === "undefined") {
      const { WebSocket: _WS } = await import("ws");
      (globalThis as unknown as Record<string, unknown>).WebSocket = _WS;
    }
  ...
  private subscribe(): void {
    ...
    /**
     * Subscription 1 — SPL Token Program.
     * Captures SetAuthority (MintTokens / FreezeAccount) and
     * SystemProgram Allocate / AllocateWithSeed inner-instructions.
     */
    ...
    /**
     * Subscription 2 — Token Metadata Program.
     * Captures UpdateMetadataAccount, UpdateV1, SetAndVerifyCollection,
     * and UpdateAsUpdateAuthorityV2 instructions (metadata hijacking).
     */
    this.ws.send(JSON.stringify({
      jsonrpc: "2.0",
      id: 2,
      method: "logsSubscribe",
      params: [
        { mentions: [TOKEN_METADATA_PROGRAM_ID] },
        { commitment: "confirmed" },
      ],
    }));
  }
  ...
  private async handleMessage(raw: string): Promise<void> {
    ...
    // Subscription confirmations — save subscription IDs.
    if (typeof msg?.result === "number") {
      if (msg?.id === 1) {
        ...
        this.metadataSubscriptionId = msg.result;
        console.log(
          `[PostLaunchWatcher] Subscribed to Token Metadata Program logs (sub ID: ${this.metadataSubscriptionId}).`,
        );
      }
      return;
    }

    // Notification — inspect logs.
    ...
    // Fast pre-filter: detect candidate logs.
    const mentionsSetAuthority = logs.some((l: string) => l.includes("SetAuthority"));
    // SystemProgram allocate / realloc surface markers — multiple variants
    // because CPI-issued resizes show up under different program logs.
    ...
    // Token Metadata Program update instructions
    ...
    if (mentionsResize) {
      const resizeAlerts = await extractAccountResizes(
        signature,
        this.trackedMints,
        this.apiKey,
      );
    ...
    if (mentionsMetadataUpdate) {
      const metaAlerts = await extractMetadataHijacks(
        signature,
        this.trackedMints,
        this.apiKey,
      );
    ...
    // -------------------------------------------------------------------------
    // CPI-Depth / Transaction Bloat & Re-routing Monitor
    //
    // Recursively traverse `meta.innerInstructions` to measure the maximum
    // CPI nesting depth. Depth >= 3 flags is_path_obfuscated = TRUE; depth
    // >= 4 (Solana protocol maximum) is treated as 'Extreme Obfuscation' and
    // fires a 'Critical Risk' alert.
    // -------------------------------------------------------------------------
    try {
      ...
      const obfAlert = extractPathObfuscation(tx, signature, this.trackedMints);
