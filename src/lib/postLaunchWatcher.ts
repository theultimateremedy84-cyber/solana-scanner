...
// FIX (Supabase data-health scan, 2026-07-08): well-known DEX/router/aggregator
// programs legitimately produce deep CPI nesting as part of normal multi-hop
// swap routing — that is NOT evidence of a "Programmable Rug". Without this
// exclusion, extractPathObfuscation() below can end up recording one of these
// program IDs as the "touched mint" whenever it shows up in a tracked token's
// swap route, which produced 145 false-positive "critical" path_obfuscation
// alerts against Jupiter's aggregator program in a single day (29% of all
// alerts in that window). Real per-mint obfuscation alerts should never
// target one of these addresses, so they are excluded from
// `touchedMint` resolution entirely.
const KNOWN_NON_MINT_PROGRAM_IDS = new Set<string>([
  "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN", // Jupiter Aggregator v6
  "JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4", // Jupiter Aggregator v4
  "675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8", // Raydium AMM v4
  "5quBtoiQqxF9Jv6KYKctB59NT3gtJD2Y65kdnB1Uev3h", // Raydium AMM v4 (Fixed)
  "CAMMCzo5YL8w4VFF8KVHrK22GGUsp5VTaW7grrKgrWqK", // Raydium CLMM
  "whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc",  // Orca Whirlpools
  "6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P", // Pump.fun AMM
]);

/** True when `address` is a known DEX/router/aggregator program, not a token mint. */
function isKnownNonMintProgram(address: string | null | undefined): boolean {
  return !!address && KNOWN_NON_MINT_PROGRAM_IDS.has(address);
}
...
function extractPathObfuscation(
  ...
  for (const a of accountKeys) {
    ...
    // FIX (Supabase data-health scan, 2026-07-08): skip well-known DEX/router
    // program IDs here — they legitimately appear in accountKeys for any
    // multi-hop swap and must never be reported as the "touched mint" that
    // triggered a path-obfuscation alert. See KNOWN_NON_MINT_PROGRAM_IDS
    // above for the full explanation (Jupiter false-positive incident).
    if (pubkey && !isKnownNonMintProgram(pubkey) && trackedMints.has(pubkey)) {
      touchedMint = pubkey;
      break;
    }
  }
