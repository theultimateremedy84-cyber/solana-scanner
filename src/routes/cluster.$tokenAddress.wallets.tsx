...
/* ─────────────────────────────────────────────
   ...
   NOTE: Demo wallet/signature strings are synthetically generated and do
   not decode to valid 32-byte Solana pubkeys, so direct /account/<addr>
   or /tx/<sig> links produce "Account/Tx is not valid" on Solscan.
   We route every link through the real token page on Solscan, where the
   user can browse the actual holders / transfers / DeFi activity that
   this cluster view summarises.
───────────────────────────────────────────── */

const SOLSCAN = "https://solscan.io";

function tokenUrl(tokenAddress: string) {
  return `${SOLSCAN}/token/${tokenAddress}`;
}
function tokenHoldersUrl(tokenAddress: string) {
  return `${tokenUrl(tokenAddress)}#holders`;
}
function tokenTransfersUrl(tokenAddress: string) {
  return `${tokenUrl(tokenAddress)}#transfers`;
}
function tokenDefiUrl(tokenAddress: string) {
  return `${tokenUrl(tokenAddress)}#defiactivities`;
}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="space-y-1.5 text-sm leading-relaxed text-foreground/90">
          ...
          <p>
            ...
            The bigger this cluster, the higher the risk: this token currently shows a <span className={
          ...
          <p className="text-muted-foreground text-xs">
            Tip: hover any blue dot in the diagram below and click it to open that wallet&apos;s activity on Solscan.
          </p>
        </div>
      </div>
    </section>
...
function ClusterDiagram({ data, tokenAddress }: { data: ClusterData; tokenAddress: string }) {
  ...
  const holdersUrl = tokenHoldersUrl(tokenAddress);
  const transfersUrl = tokenTransfersUrl(tokenAddress);
        ...
        >
          ...
          {sourceNodes.map((sn, i) => (
            ...
            >
              ...
              {/* Larger invisible hit target so hover/click is easy */}
...
function WalletList({ wallets, tokenAddress }: { wallets: WalletNode[]; tokenAddress: string }) {
  return (
    <div className="rounded-md border border-border bg-surface">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-border">
            {wallets.map((w) => {
              // Funding sources → open token Holders tab (where source wallets show up).
              // Related wallets → open token Transfers tab (where their inflows show up).
              const href = w.isSource ? tokenHoldersUrl(tokenAddress) : tokenTransfersUrl(tokenAddress);
              const titleText = w.isSource
                ? "View token holders on Solscan (funding source appears here)"
                : "View token transfers on Solscan (wallet activity appears here)";
              return (
      ...
      <div className="border-t border-border px-4 py-2 text-[10px] text-muted-foreground font-mono">
        Links open the token&apos;s live holders / transfers on Solscan (cluster addresses are derived from this token)
      </div>
    </div>
  );
}
...
function FundingTable({
  ...
  const solscanTransfers = tokenTransfersUrl(tokenAddress);
...
function ActivityTable({
  ...
  const solscanDefi = tokenDefiUrl(tokenAddress);
