...
/* ─────────────────────────────────────────────
   Deterministic data generation (same helpers
   used by mockScan.ts so same address → same data)
───────────────────────────────────────────── */
...
function generateClusterData(tokenAddress: string): ClusterData {
  ...
  // Same count logic as mockScan.ts clusterWallets
  ...
  // Related wallets funded by one of the sources
  ...
  // Funding transactions (source → wallet)
    ...
    <div className="min-h-screen bg-background px-3 py-3 text-foreground sm:px-6 sm:py-6 lg:px-8">
      <div className="hud-frame relative mx-auto max-w-7xl border border-border bg-surface">
        ...
        <main className="space-y-6 px-5 py-6 sm:px-8">
          ...
          {/* ── Data sources ── */}
          <SourcePanel sources={data.infoSources} />

          {/* ── Relationship diagram ── */}
          <ClusterDiagram data={data} />

          {/* ── Wallet list ── */}
          <WalletList wallets={allWallets} />

          {/* ── Fund transactions ── */}
          <FundingTable txs={data.fundingTxs} wallets={allWallets} />

          {/* ── Wallet activity ── */}
          <ActivityTable txs={data.walletTxs} />
        </main>
      </div>
    </div>
  );
...
function SourcePanel({ sources }: { sources: string[] }) {
  return (
...
function ClusterDiagram({ data }: { data: ClusterData }) {
  ...
  // Token node at center
  const tokenNode = { x: cx, y: cy, label: "TOKEN", isToken: true };

  // Source wallets on inner ring
  const innerR = 110;
  ...
  const tokenEdges = sourceNodes.map((sn) => ({
    x1: tokenNode.x, y1: tokenNode.y, x2: sn.x, y2: sn.y, type: "token-source",
  }));

  // Edges: source→wallet
  const fundingEdges = walletNodes.map((wn) => {
    const srcNode = sourceNodes.find((sn) => sn.wallet.address === wn.wallet.fundedBy);
    if (!srcNode) return null;
    return { x1: srcNode.x, y1: srcNode.y, x2: wn.x, y2: wn.y, type: "funding" };
  }).filter(Boolean) as { x1: number; y1: number; x2: number; y2: number; type: string }[];
  ...
  return (
    <div className="rounded-md border border-border bg-surface">
      ...
      <div className="border-t border-border px-4 py-2 text-[10px] text-muted-foreground font-mono">
        Dashed lines = token ↔ funding source &nbsp;·&nbsp; Solid amber lines = funding flow &nbsp;·&nbsp; Blue dots = related wallets
      </div>
    </div>
  );
}
...
function WalletList({ wallets }: { wallets: WalletNode[] }) {
  return (
    <div className="rounded-md border border-border bg-surface">
      <div className="border-b border-border px-4 py-2.5">
...
function FundingTable({ txs, wallets }: { txs: FundTx[]; wallets: WalletNode[] }) {
...
function ActivityTable({ txs }: { txs: WalletTx[] }) {
  return (
    <div className="rounded-md border border-border bg-surface">
      <div className="border-b border-border px-4 py-2.5">
        <div className="text-xs font-semibold uppercase tracking-wide">
          ...
          <tbody className="divide-y divide-border">
            {txs.map((tx, i) => (
              <tr key={tx.signature + i} className="hover:bg-surface-2 transition-colors">
                ...
                <td className="px-4 py-2.5">
                  <span className={tx.direction === "in"
