...
export const APIRoute = createAPIFileRoute("/api/process-jobs")({
  POST: async ({ request }) => {
    ...
    // BACKLOG_MODE guard — kept in sync with the production handler
    // (src/lib/api/process-jobs-handler.ts). This route file is not the
    // production entry point on Railway, but it is still a real, reachable
    // execution path in local/dev environments, so it must not silently
    // bypass BACKLOG_MODE.
