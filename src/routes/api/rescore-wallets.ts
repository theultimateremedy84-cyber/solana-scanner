...
//
// Authentication: CRON_SECRET header required (hard error if env var missing).
//
// Usage:
...
export async function POST(request: Request): Promise<Response> {
  ...
  // SECURITY FIX: CRON_SECRET is now required. If the env var is not set the
  // endpoint rejects every request rather than allowing unauthenticated access.
...
export async function GET(): Promise<Response> {
  return json({
    ...
    auth:    "Header: x-cron-secret: <CRON_SECRET>  (required — CRON_SECRET must be set in env)",
    body:    {
