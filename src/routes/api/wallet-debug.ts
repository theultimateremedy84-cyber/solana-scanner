...
// Requires header: x-cron-secret: <CRON_SECRET>  (same secret as cron endpoints)
...
export const APIRoute = createAPIFileRoute("/api/wallet-debug")({
  GET: async ({ request }) => {
    ...
    if (cronSecret && (!incomingSecret || incomingSecret !== cronSecret)) {
      return new Response(
        JSON.stringify({ ok: false, error: "Unauthorized — add header: x-cron-secret: <CRON_SECRET>" }),
