...
export const APIRoute = createAPIFileRoute("/api/wallet-test")({
  GET: async ({ request }) => {
    // ── Auth guard — fail-closed (matches wallet-debug.ts pattern) ───────────
    // Rejects ALL requests when CRON_SECRET is unset (misconfigured deployment).
    const cronSecret     = process.env.CRON_SECRET;
    const incomingSecret = request.headers.get("x-cron-secret");
    if (!cronSecret) {
      return new Response(
        JSON.stringify({ ok: false, error: "Service misconfigured: CRON_SECRET not set" }),
