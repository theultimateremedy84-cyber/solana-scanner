...
//
// FIX APPLIED (July 2026 re-audit):
//   When resetting jobs to 'pending', the original script did not reset the
//   `attempts` counter. A job that had failed twice would be re-queued still
//   showing 2 attempts, and would be treated as permanently failed (≥ 3 attempts)
//   on its very next failure — consuming the retry budget immediately.
//   Fix: `attempts` is now reset to 0 along with status and error_message.
//
// Usage:
      ...
      const { error } = await sb
        ...
        .upsert(
          slice.map((j) => ({
            id:            j.id,
            status:        "pending",
            // FIX: Reset attempts to 0 so the job gets its full retry budget.
            // The original code left attempts unchanged, meaning a job that had
            // failed twice would be treated as permanently failed on its next
            // failure without any additional retry attempts.
            attempts:      0,
            error_message: null,
